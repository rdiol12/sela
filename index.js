...
import { startWhatsApp, closeWhatsAppSocket } from './lib/whatsapp.js';
...
async function onShutdown(signal) {
  ...
  // 0. Close WA socket FIRST — prevents conflict/replaced on next process
  closeWhatsAppSocket();
  ...
  // Give Telegram API a moment to send, then kill entire process tree
  setTimeout(() => {
    if (process.platform === 'win32') {
      // Kill entire process tree — prevents orphaned MCP servers, Claude CLI, ws-gateway
      try { execSync(`taskkill /T /F /PID ${process.pid}`, { timeout: 5000, stdio: 'ignore' }); } catch {}
    }
    process.exit(0);
  }, 1500);
  // Hard kill fallback: if process.exit(0) doesn't work (Windows zombie), force-kill tree after 8s
  setTimeout(() => {
    log.warn('Graceful exit timed out — force-killing process tree');
    if (process.platform === 'win32') {
      try { execSync(`taskkill /T /F /PID ${process.pid}`, { timeout: 5000, stdio: 'ignore' }); } catch {}
    }
...
process.on('unhandledRejection', (err) => {
  if (err?.code === 'EPIPE' || String(err).includes('EPIPE')) return; // PM2 IPC — non-fatal
...
process.on('uncaughtException', (err) => {
  // EPIPE from PM2 IPC transport is non-fatal — swallow to prevent crash cascade
  if (err?.code === 'EPIPE' || err?.message?.includes('EPIPE')) {
    log.warn({ err: err.message }, 'Swallowed EPIPE (PM2 IPC broken pipe)');
    return;
  }
...
// Load optional modules — skip unreal, keep maplestory and hattrick alive
await loadModules({ skip: ['unreal'] });

// Start agent loop (core + project cycles)
