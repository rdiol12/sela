...
export class PersistentProcess {
  ...
  constructor(name, sessionId, systemPrompt, opts = {}) {
    ...
    this.mcpConfig = opts.mcpConfig || MCP_CONFIG;
  ...
  _spawnProcess() {
    ...
    this.fullText = '';
    this.resultEvent = null;
    this._cumulativeCostUsd = 0; // Reset cumulative cost on new process spawn
  ...
  _finishCurrentMessage() {
    ...
    // Prefer fullText (accumulated from all text blocks) over resultEvent.result
    // because resultEvent.result may strip inline XML tags like <tool_call> that
    // the agent-loop needs to parse and execute.
    const replyText = this.fullText || this.resultEvent?.result || '';
    ...
    if (this.resultEvent?.is_error) {
      ...
      this.currentMessage.resolve({
        ...
        costUsd: this._computeDeltaCost(this.resultEvent?.total_cost_usd || 0),
  ...
  // ─── Cost Delta ──────────────────────────────────────────────────────────
  // Claude CLI total_cost_usd is cumulative for the session. We need per-message deltas.
  _computeDeltaCost(totalCostUsd) {
    const delta = Math.max(0, totalCostUsd - (this._cumulativeCostUsd || 0));
    this._cumulativeCostUsd = totalCostUsd;
    return delta;
  }
  ...
  _killProcess() {
    ...
    try {
      this.proc.stdin.end();
      // On Windows, proc.kill() only kills the direct child — not grandchildren (MCP servers).
      // Use taskkill /T to kill the entire process tree and prevent zombie accumulation.
...
export function initPersistentProcess(sid, sysPrompt, extraArgs = []) {
  waInstance = new PersistentProcess('wa', sid, sysPrompt, { extraArgs, mcpConfig: config.mcpConfigCorePath });
}
