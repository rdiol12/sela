// Project-scoped goals board — same as /board but filtered to a single project's goals
export function projectBoardHtml(projectId, projectTitle, projectSlug) {
  ...
  const eSlug = (projectSlug || projectId).replace(/'/g, "\\'").replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<title>${eTitle} — Goals Board</title>
...
<script>
  ...
  async function loadTasks() {
    try {
      var results = await Promise.all([
        api('/goals?all=1&project=' + encodeURIComponent(PROJECT_SLUG)),
        api('/goals?all=1&parentGoalId=' + encodeURIComponent(PROJECT_ID))
      ]);
      var seen = {};
      var merged = [];
      (results[0].goals || []).concat(results[1].goals || []).forEach(function(g) {
      ...
      taskStore = (d.goals || []).map(function(g) {
        ...
        return { id: g.id, title: g.title, status: st, assignee: g.source || 'user', priority: g.priority || 'medium', raw: g };
      });
      renderKanban();
    } catch(e) {
  ...
  async function addTask() {
    ...
    try {
      await api('/projects/' + encodeURIComponent(PROJECT_ID) + '/goals', {
        method: 'POST', body: JSON.stringify({ title: title, priority: priority })
      });
    ...
    try {
      ...
      var html = '<div style="margin-bottom:11px;">' +
        ...
        '<div style="color:var(--text3);font-size:10px;display:flex;align-items:center;gap:8px;">' +
          ...
          (g.progress||0) + '% \\u00b7 ' + (g.priority||'medium') +
        '</div>' +
      '</div>';
      ...
      if (g.milestones && g.milestones.length) {
        ...
        g.milestones.forEach(function(m) {
          ...
          html += '<div class="row" style="cursor:' + (m.status==='completed'?'default':'pointer') + '"' +
            (m.status==='completed' ? '' : ' onclick="completeMilestone(\\'' + esc(g.id) + '\\',\\'' + esc((m.id||m.title).replace(/'/g,"\\\\'")) + '\\')" title="Click to complete"') + '>' +
