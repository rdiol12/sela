...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Agent cycle toggle */
  .agent-toggle { display: inline-flex; align-items: center; gap: 6px; cursor: pointer; font-size: 10px; color: var(--text3); white-space: nowrap; }
  .agent-toggle:hover { color: var(--text2); }
  .agent-switch { position: relative; width: 28px; height: 14px; background: var(--border2); border-radius: 7px; transition: background 0.2s; }
  .agent-switch.on { background: var(--green); }
  .agent-switch::after { content: ''; position: absolute; top: 2px; left: 2px; width: 10px; height: 10px; background: var(--text); border-radius: 50%; transition: transform 0.2s; }
  .agent-switch.on::after { transform: translateX(14px); }
  .agent-cfg { display: flex; align-items: center; gap: 6px; margin-top: 8px; padding: 8px 10px; background: var(--bg); border: 1px solid var(--border); border-radius: 6px; font-size: 10px; color: var(--text2); }
  .agent-cfg label { color: var(--text3); text-transform: uppercase; letter-spacing: 0.3px; }
  .agent-cfg input, .agent-cfg select { background: var(--surface2); border: 1px solid var(--border); border-radius: 3px; color: var(--text); font-family: var(--font-mono); font-size: 10px; padding: 3px 6px; outline: none; width: 50px; }
  .agent-cfg select { width: 80px; }
    ...
    return '<div class="card" data-id="' + esc(p.id) + '">' +
      '<div class="card-header" onclick="toggleProject(\\'' + esc(p.id) + '\\')">' +
        ...
        '<div class="project-meta">' +
          ...
          '<span class="agent-toggle" onclick="event.stopPropagation();toggleAgentCycle(\\'' + esc(p.slug || p.id) + '\\',' + (p.agentCycle && p.agentCycle.enabled ? 'false' : 'true') + ',this)">' +
            '<span class="agent-switch' + (p.agentCycle && p.agentCycle.enabled ? ' on' : '') + '"></span>Agent' +
          '</span>' +
...
function renderProjectBody(p) {
  ...
  // Agent cycle config (shown when enabled)
  if (ac.enabled) {
    html += '<div class="agent-cfg" id="acfg-' + esc(slug) + '">' +
      '<label>Interval</label>' +
      '<input type="number" min="1" max="60" value="' + (ac.intervalMin || 10) + '" onchange="updateAgentCycle(\\'' + esc(slug) + '\\',{intervalMin:+this.value})">' +
      '<span>min</span>' +
      '<label style="margin-left:8px">Model</label>' +
      '<select onchange="updateAgentCycle(\\'' + esc(slug) + '\\',{model:this.value})">' +
        '<option value="sonnet"' + ((ac.model || 'sonnet') === 'sonnet' ? ' selected' : '') + '>Sonnet</option>' +
        '<option value="haiku"' + (ac.model === 'haiku' ? ' selected' : '') + '>Haiku</option>' +
        '<option value="opus"' + (ac.model === 'opus' ? ' selected' : '') + '>Opus</option>' +
      '</select>' +
    '</div>';
  }
...
function toggleAgentCycle(slug, enabled, el) {
  fetch('/api/projects/' + encodeURIComponent(slug) + '/agent-cycle', {
    method: 'POST',
  ...
  .then(function(d) {
    if (d.success) {
      showToast(slug + ' agent cycle ' + (enabled ? 'enabled' : 'disabled'), 'success');
      loadProjects();
    } else {
      showToast('Failed to toggle agent cycle', 'error');
    }
  })
  .catch(function() { showToast('Network error', 'error'); });
}

function updateAgentCycle(slug, changes) {
  fetch('/api/projects/' + encodeURIComponent(slug) + '/agent-cycle')
  .then(function(r) { return r.json(); })
  .then(function(d) {
    var cycle = d.cycle || {};
    var updated = { enabled: true, intervalMin: changes.intervalMin || cycle.intervalMin || 10, model: changes.model || cycle.model || 'sonnet' };
    return fetch('/api/projects/' + encodeURIComponent(slug) + '/agent-cycle', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  .then(function(r) { return r.json(); })
  .then(function(d) {
    if (d.success) showToast('Updated ' + slug + ' cycle config', 'success');
  })
