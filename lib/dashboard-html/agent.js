...
<div class="layout">
  ...
  <div class="panel-left">
    ...
    <div class="card" id="projectCyclesCard" style="display:none">
      <div class="card-header">Project Cycles <span id="projectCycleCount" class="badge badge-gray">0</span></div>
      <div class="card-body" style="max-height:40vh;overflow-y:auto">
        <div id="projectCycleList" class="signal-list">
          <div class="empty-state">No project cycles configured</div>
        </div>
      </div>
    </div>
...
const EVENT_CONFIG = {
  ...
  'project:cycle:start':      { icon: '&#9654;', cls: 'start',    title: 'Project Cycle Start' },
  'project:cycle:skip':       { icon: '&#8212;', cls: 'skip',     title: 'Project Cycle Skip' },
  'project:cycle:phase2':     { icon: '&#9881;', cls: 'phase2',   title: 'Project Claude Spawn' },
  'project:cycle:actions':    { icon: '&#9889;', cls: 'complete', title: 'Project Actions' },
  'project:cycle:complete':   { icon: '&#10003;', cls: 'complete', title: 'Project Cycle Done' },
  'project:cycle:error':      { icon: '&#10007;', cls: 'error',   title: 'Project Cycle Error' },
};
...
function eventDetail(event, data) {
  switch (event) {
    ...
    case 'agent:cycle:complete': {
      var cc = data.cycleCount || (data.replyFile ? (data.replyFile.match(/cycle-(\\d+)-reply/) || [])[1] : '');
      return '$' + (data.costUsd || 0).toFixed(4) + ' | ' + (data.waMessageCount || 0) + ' msg | ' +
        (data.followupCount || 0) + ' followup | next ' + (data.nextCycleMinutes || '?') + 'min' +
        (cc ? ' | <span class="prompt-link" onclick="viewReply(this)" data-cycle="' + cc + '">view reply</span>' : '');
    }
    case 'agent:cycle:error': return data.error || 'Unknown error';
    case 'agent:cycle:actions': {
      var cc2 = data.cycleCount || (data.replyFile ? (data.replyFile.match(/cycle-(\\d+)-reply/) || [])[1] : '');
      return ((data.actions || []).join('; ') || 'No actions') +
        (cc2 ? ' <span class="prompt-link" onclick="viewReply(this)" data-cycle="' + cc2 + '">view reply</span>' : '');
    }
    case 'agent:cycle:goal_created': return (data.title || 'Untitled') + ' (id: ' + (data.goalId || '?') + ')';
    case 'agent:cli:spawn': {
      var ctx = 'user msg';
      var preview = data.promptPreview || '';
      if (preview.indexOf('AGENT_CYCLE [PROJECT:') !== -1) {
        var pm = preview.match(/PROJECT:\\s*([^\\]]+)/);
        ctx = pm ? pm[1].trim() : 'project';
      } else if (preview.indexOf('AGENT_CYCLE') !== -1) {
        ctx = 'main loop';
      }
      return '<b>' + escHtml(ctx) + '</b> | ' + (data.source || 'cli') + ' | ' + (data.model || '?') + ' | <span class="prompt-link" onclick="viewCliPrompt(this)" data-prompt-id="' + escHtml(data.promptId || '') + '">' + (data.promptLen || 0) + ' chars</span>';
    }
    case 'project:cycle:start': return 'Cycle #' + (data.cycleCount || '?');
    case 'project:cycle:skip': return (data.reason || 'unknown').replace(/_/g, ' ');
    case 'project:cycle:phase2': return (data.model || '?') + ' | <span class="prompt-link" onclick="viewProjectCyclePrompt(this)" data-file="' + escHtml(data.promptFile || '') + '" data-project="' + escHtml(data.project || '') + '">' + (data.promptLen || 0) + ' chars</span> | $' + (data.costUsd || 0).toFixed(4);
    case 'project:cycle:actions': {
      var pcc = data.cycleCount || (data.replyFile ? (data.replyFile.match(/cycle-(\\d+)-reply/) || [])[1] : '');
      return ((data.actions || []).join('; ') || 'No actions') +
        (pcc && data.project ? ' <span class="prompt-link" onclick="viewReply(this)" data-cycle="' + pcc + '" data-project="' + escHtml(data.project) + '">view reply</span>' : '');
    }
    case 'project:cycle:complete': {
      var pcc2 = data.cycleCount || (data.replyFile ? (data.replyFile.match(/cycle-(\\d+)-reply/) || [])[1] : '');
      return '#' + (data.cycleCount || '?') + ' | ' + (data.actionCount || 0) + ' actions | ' + (data.followupCount || 0) + ' followups' +
        (pcc2 && data.project ? ' | <span class="prompt-link" onclick="viewReply(this)" data-cycle="' + pcc2 + '" data-project="' + escHtml(data.project) + '">view reply</span>' : '');
    }
...
function updateStatus(al) {
  ...
  // Project cycles
  if (al.projectCycles) renderProjectCycles(al.projectCycles);
...
function renderProjectCycles(pc) {
  var card = document.getElementById('projectCyclesCard');
  var list = document.getElementById('projectCycleList');
  var countEl = document.getElementById('projectCycleCount');
  if (!pc || typeof pc !== 'object') { card.style.display = 'none'; return; }
  ...
  list.innerHTML = keys.map(function(k) {
    ...
    var badge = p.paused ? '<span class="badge badge-yellow">paused</span>' : running ? '<span class="badge badge-green">active</span>' : '<span class="badge badge-yellow">idle</span>';
    var interval = p.intervalMin ? p.intervalMin + 'min' : '?';
    var cycles = p.cycleCount || 0;
    var sigCount = (p.lastSignals || []).length;
    var nextIn = '';
    if (p.nextCycleAt) {
      var diff = Math.round((new Date(p.nextCycleAt).getTime() - Date.now()) / 1000);
      if (diff > 0) { var m = Math.floor(diff/60), s = diff%60; nextIn = m + ':' + (s<10?'0':'') + s; }
      else nextIn = 'now';
    }
    var lct = p.lastCycleTokens;
    var tokenInfo = '';
    if (lct) {
      var totalK = ((lct.input || 0) + (lct.output || 0)) / 1000;
      tokenInfo = '<span>' + totalK.toFixed(1) + 'K tok</span>' + (lct.model ? '<span>' + lct.model + '</span>' : '');
    }
    return '<a href="/agent/' + encodeURIComponent(k) + '" style="text-decoration:none;color:inherit;display:block">' +
      '<div class="signal-item" style="cursor:pointer;border-left-color:var(--accent);transition:background 0.15s" onmouseover="this.style.background=\\'var(--border)\\'" onmouseout="this.style.background=\\'\\'">' +
        '<div style="display:flex;justify-content:space-between;align-items:center">' +
          '<span style="font-weight:600;font-size:13px;text-transform:capitalize">' + escHtml(k) + '</span>' +
          badge +
        '</div>' +
        '<div style="display:flex;gap:12px;margin-top:4px;font-size:10px;color:var(--text2)">' +
          ...
          tokenInfo +
          (nextIn ? '<span style="margin-left:auto;color:var(--cyan)">next ' + nextIn + '</span>' : '') +
        '</div>' +
      '</div></a>';
  }).join('');
}
...
function renderLog() {
  ...
  el.innerHTML = eventLog.map(function(e) {
    ...
    var title = cfg.title;
    if (e.data && e.data.project) title += ' [' + escHtml(e.data.project) + ']';
    else if (e.event.startsWith('agent:cycle:')) title += ' [main]';
    return '<div class="event-entry">' +
      '<div class="event-icon ' + cfg.cls + '">' + cfg.icon + '</div>' +
      '<div class="event-content">' +
        '<div class="event-title">' + title + '</div>' +
        '<div class="event-detail">' + (['agent:cycle:phase2','agent:cycle:complete','agent:cycle:actions','agent:cli:spawn','project:cycle:phase2','project:cycle:actions','project:cycle:complete'].indexOf(e.event) !== -1 ? eventDetail(e.event, e.data) : escHtml(eventDetail(e.event, e.data))) + '</div>' +
      '</div>' +
...
function viewReply(el) {
  ...
  if (!cycle) { alert('No cycle number'); return; }
  var url = project
    ? '/api/cycle-diffs/project/' + encodeURIComponent(project) + '/' + cycle + '/reply'
    : '/api/cycle-diffs/' + cycle + '/reply';
  var title = project
    ? '[' + project + '] Cycle ' + cycle + ' Reply'
    : 'Cycle ' + cycle + ' Reply';
  fetch(url)
    .then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
    .then(function(data) {
      if (data.error) { alert('Reply not found: ' + data.error); return; }
      showPromptModal(title + ' (' + (data.reply || '').length + ' chars)', data.reply || '');
    })
    .catch(function(err) { alert('Failed to load reply: ' + err.message); });
}

function viewProjectCyclePrompt(el) {
  var file = el.getAttribute('data-file');
  var project = el.getAttribute('data-project');
  if (!file) return;
  var m = file.match(/project-([a-z0-9_-]+)-cycle-(\\d+)-prompt/);
  if (!m) return;
  var proj = m[1];
  var cycle = m[2];
  fetch('/api/cycle-diffs/project/' + encodeURIComponent(proj) + '/' + cycle + '/prompt')
    .then(function(r) {
      if (!r.ok) throw new Error('HTTP ' + r.status);
      return r.json();
    })
    .then(function(data) {
      ...
      showPromptModal('[' + (project || proj) + '] Cycle ' + cycle + ' Prompt (' + (data.prompt || '').length + ' chars)', data.prompt || '');
    })
    .catch(function(err) { alert('Failed to load prompt: ' + err.message); });
}
...
function connectWs() {
  ...
  wsConn.onmessage = function(e) {
    try {
      ...
      if (msg.type === 'event' && msg.event && (msg.event.startsWith('agent:cycle:') || msg.event.startsWith('agent:cli:') || msg.event.startsWith('project:cycle:'))) {
...
function togglePause() {
  ...
  btn.disabled = true;
  btn.style.opacity = '0.5';
  var endpoint = _mainPaused ? '/api/agent-loop/resume' : '/api/agent-loop/pause';
  fetch(endpoint, { method: 'POST' })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      _mainPaused = data.paused;
      updatePauseBtn();
      btn.disabled = false;
      btn.style.opacity = '1';
      loadDetail();
