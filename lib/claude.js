...
/** Kill a process and its entire tree on Windows (prevents zombie MCP servers). */
...
const ALL_MARKERS_RE = /\[(CRON_ADD|CRON_DELETE|CRON_TOGGLE|CRON_RUN|SEND_FILE|SEND_VIDEO|TOOL_CALL):[^\]]*\]/gs;
...
async function executeMarkers(text) {
  ...
  // SEND_VIDEO: single video path OR comma-separated frame paths
  for (m of text.matchAll(/\[SEND_VIDEO:\s*(.+?)\]/g)) {
    const val = m[1].trim();
    if (val.includes(',')) {
      videosToSend.push(val.split(',').map(p => p.trim()));
    } else {
      videosToSend.push(val);
    }
  }
  ...
  return { filesToSend, videosToSend, actionCount: actions.length, toolResultMessages };
}
...
- [SEND_VIDEO: path.mp4] -- send a video inline on WhatsApp
- [SEND_VIDEO: frame1.png, frame2.png, ...] -- convert frame images to mp4 and send as video
...
function spawnClaude(args, stdinContent, onTextDelta, onToolUse, signal, { timeoutMs } = {}) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(maxTimer);
      clearInterval(activityCheck);
      // Clean up any surviving child processes (MCP servers etc.)
      killProcessTree(proc);
...
export async function chat(history, onChunk, botContext = {}, { tier = 2, onToolUse } = {}) {
  ...
  const { filesToSend, videosToSend, actionCount, toolResultMessages } = await executeMarkers(result.text);
  if (actionCount > 0) {
...
export async function chatOneShot(prompt, onChunk, modelOverride = null, { cronId, cronName, cronState, onToolUse, session } = {}) {
  ...
  return {
    reply: cleanReply, claudeMs: cliMs, filesToSend, videosToSend, costUsd: result.costUsd || 0,
