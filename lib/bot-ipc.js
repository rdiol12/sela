...
import { getAgentLoopStatus, getAgentLoopDetail, getCycleDiffs, markCycleReviewed, triggerCycleNow, getProjectCycleDetail, triggerProjectCycleNow, pauseMainCycle, resumeMainCycle, pauseProjectCycle, resumeProjectCycle } from './agent-loop.js';
import { listIdeas, addIdea, updateIdea, removeIdea, seedIfEmpty as seedIdeas } from './ideas.js';
import { listProjects, getProject, createProject, importProject, addGoalToProject, scanDirectory, getProjectAgentCycle, setProjectAgentCycle } from './projects.js';
import { getModuleApiRoutes } from './module-loader.js';
import { getMemoryDashboard, getTier, getHeapStats } from './memory-guardian.js';
import { sendFileToJid, sendVideoToJid, sendToJid } from './whatsapp.js';
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    // --- Project Agent Cycle Toggle ---
    const projectCycleMatch = path.match(/^\/projects\/([^/]+)\/agent-cycle$/);
    if (projectCycleMatch && method === 'GET') {
      const slug = decodeURIComponent(projectCycleMatch[1]);
      const cycle = getProjectAgentCycle(slug);
      return jsonResponse(res, { slug, cycle: cycle || { enabled: false, intervalMin: 10, model: 'sonnet' } });
    }
    if (projectCycleMatch && method === 'POST') {
      const slug = decodeURIComponent(projectCycleMatch[1]);
      const body = await readBody(req);
      const cycle = setProjectAgentCycle(slug, {
        enabled: body.enabled !== undefined ? body.enabled : true,
        intervalMin: body.intervalMin || 10,
        model: body.model || 'sonnet',
      });
      return jsonResponse(res, { success: true, slug, cycle });
    }
    ...
    if (proposalReviewMatch && method === 'POST') {
      ...
      return; // Prevent fall-through to 404 (was causing "Cannot write headers" errors)
    }
    ...
    if (path === '/send-file' && method === 'POST') {
      const body = await readBody(req);
      if (!body.filePath) return jsonResponse(res, { error: 'Missing filePath' }, 400);
      const jid = body.jid || config.allowedJid;
      try {
        await sendFileToJid(jid, body.filePath);
        return jsonResponse(res, { success: true, filePath: body.filePath });
      } catch (err) {
        return jsonResponse(res, { error: err.message }, 500);
      }
    }
    ...
    if (path === '/send-message' && method === 'POST') {
      const body = await readBody(req);
      if (!body.text) return jsonResponse(res, { error: 'Missing text' }, 400);
      const jid = body.jid || config.allowedJid;
      try {
        await sendToJid(jid, body.text);
        return jsonResponse(res, { success: true, jid, length: body.text.length });
      } catch (err) {
        return jsonResponse(res, { error: err.message }, 500);
      }
    }

    // --- Send video via WhatsApp (supports frame arrays → mp4 conversion) ---
    if (path === '/send-video' && method === 'POST') {
      const body = await readBody(req);
      if (!body.filePath && !body.frames) return jsonResponse(res, { error: 'Missing filePath or frames' }, 400);
      const jid = body.jid || config.allowedJid;
      try {
        const input = body.frames || body.filePath; // frames = array of PNGs, filePath = single mp4
        await sendVideoToJid(jid, input, { caption: body.caption, fps: body.fps, loop: body.loop, gifPlayback: body.gifPlayback });
    ...
    if (path === '/healthz' && method === 'GET') {
      ...
      const qmdStatus = getAgentLoopDetail()?.qmdSync;
      ...
      return res.end(JSON.stringify({
        ...
        ...(qmdStatus && { qmd_sync: { running: qmdStatus.running, lastResult: qmdStatus.lastResult, failures: qmdStatus.failures } }),
      }));
    }
    ...
    if (path === '/agent-loop/pause' && method === 'POST') {
      return jsonResponse(res, pauseMainCycle());
    }

    if (path === '/agent-loop/resume' && method === 'POST') {
      return jsonResponse(res, resumeMainCycle());
    }

    const projectAgentMatch = path.match(/^\/agent-loop\/projects\/([^/]+)$/);
    if (projectAgentMatch && method === 'GET') {
      const projectId = decodeURIComponent(projectAgentMatch[1]);
      const detail = getProjectCycleDetail(projectId);
      if (!detail) return jsonResponse(res, { error: 'Project not found' }, 404);
      return jsonResponse(res, detail);
    }

    const projectTriggerMatch = path.match(/^\/agent-loop\/projects\/([^/]+)\/trigger$/);
    if (projectTriggerMatch && method === 'POST') {
      const projectId = decodeURIComponent(projectTriggerMatch[1]);
      const result = triggerProjectCycleNow(projectId);
      return jsonResponse(res, result, result.triggered ? 200 : 409);
    }

    const projectPauseMatch = path.match(/^\/agent-loop\/projects\/([^/]+)\/pause$/);
    if (projectPauseMatch && method === 'POST') {
      const projectId = decodeURIComponent(projectPauseMatch[1]);
      const result = pauseProjectCycle(projectId);
      return jsonResponse(res, result, result.error ? 404 : 200);
    }

    const projectResumeMatch = path.match(/^\/agent-loop\/projects\/([^/]+)\/resume$/);
    if (projectResumeMatch && method === 'POST') {
      const projectId = decodeURIComponent(projectResumeMatch[1]);
      const result = resumeProjectCycle(projectId);
      return jsonResponse(res, result, result.error ? 404 : 200);
    }
    ...
    // Project cycle prompts: /api/cycle-diffs/project/{name}/{cycle}/prompt
    const projPromptMatch = path.match(/^\/cycle-diffs\/project\/([a-z0-9_-]+)\/(\d+)\/prompt$/);
    if (projPromptMatch && method === 'GET') {
      const [, project, cycleNum] = projPromptMatch;
      const promptPath = join(config.dataDir, 'cycle-diffs', `project-${project}-cycle-${cycleNum}-prompt.txt`);
      try {
        const content = readFileSync(promptPath, 'utf-8');
        return jsonResponse(res, { project, cycle: parseInt(cycleNum, 10), prompt: content });
      } catch {
        return jsonResponse(res, { error: 'Prompt file not found' }, 404);
      }
    }
    // Main cycle reply: /api/cycle-diffs/{cycle}/reply
    const replyMatch = path.match(/^\/cycle-diffs\/(\d+)\/reply$/);
    if (replyMatch && method === 'GET') {
      const cycleNum = parseInt(replyMatch[1], 10);
      const replyPath = join(config.dataDir, 'cycle-diffs', `cycle-${cycleNum}-reply.txt`);
      try {
        const content = readFileSync(replyPath, 'utf-8');
        return jsonResponse(res, { cycle: cycleNum, reply: content });
      } catch {
        return jsonResponse(res, { error: 'Reply file not found' }, 404);
      }
    }
    // Project cycle reply: /api/cycle-diffs/project/{name}/{cycle}/reply
    const projReplyMatch = path.match(/^\/cycle-diffs\/project\/([a-z0-9_-]+)\/(\d+)\/reply$/);
    if (projReplyMatch && method === 'GET') {
      const [, project, cycleNum] = projReplyMatch;
      const replyPath = join(config.dataDir, 'cycle-diffs', `project-${project}-cycle-${cycleNum}-reply.txt`);
      try {
        const content = readFileSync(replyPath, 'utf-8');
        return jsonResponse(res, { project, cycle: parseInt(cycleNum, 10), reply: content });
      } catch {
        return jsonResponse(res, { error: 'Reply file not found' }, 404);
      }
    }
    ...
    // --- PATCH /state — update in-memory state cache from external process ---
    if (path === '/state' && method === 'PATCH') {
      const body = await readBody(req);
      const { key, value } = body;
      if (!key) return jsonResponse(res, { error: 'key required' }, 400);
      setState(key, value);
      return jsonResponse(res, { success: true, key });
    }
