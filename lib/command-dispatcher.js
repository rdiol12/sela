...
_Shortcuts: /clear /status /crons /addcron /today /files /cost /goals /brain /recap /review /rollback /export /task /tasks /plugins_`;
    ...
    case 'buildgame': {
      const { getAutonomyStatus } = await import('./unreal-autonomy.js');
      const status = await getAutonomyStatus();
      if (!status.ue5Open) {
        await sendReply(sock, sender, '❌ UE5 editor is not open. Open ShatteredCrown in UE5 first, then try again.');
        return true;
      }
      const { progress, next } = status;
      await sendReply(sock, sender,
        `🎮 *Shattered Crown Auto-Build*\n` +
        `Progress: ${progress.done}/${progress.total} tasks (${progress.pct}%)\n` +
        `Next: ${next || 'all done!'}\n\n` +
        `Building autonomously every 30min while UE5 is open.\n` +
        `I'll send Telegram updates after each build session.`
      );
      // Trigger immediate build cycle
      const { runUnrealAutonomy } = await import('./unreal-autonomy.js');
      // Reset cooldown so it runs now
      const { setState } = await import('./state.js');
      setState('unreal-autonomy', { lastRunAt: 0 });
      runUnrealAutonomy().catch(err => log.error({ err: err.message }, 'buildgame trigger failed'));
      return true;
    }
