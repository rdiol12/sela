...
import { readFileSync, writeFileSync, readdirSync, mkdirSync, statSync, rmSync, unlinkSync } from 'fs';
...
import { notify, suppressWaNotify, releaseWaNotify } from './notify.js';
...
/** Gracefully close the WA socket — call from shutdown handler. */
export function closeWhatsAppSocket() {
  if (currentSock) {
    try { currentSock.ev.removeAllListeners('connection.update'); } catch {}
...
/**
 * Send a file to a specific JID via WhatsApp.
 * @param {string} jid
 * @param {string} filePath - absolute or workspace-relative path
 * @returns {Promise<boolean>}
 */
export async function sendFileToJid(jid, filePath) {
  if (!currentSock) {
    log.warn({ jid }, 'sendFileToJid: no socket');
    return false;
  }
  await sendFileToWhatsApp(currentSock, jid, filePath, true);
  return true;
}

/**
 * Send a video to a WhatsApp JID. The video plays inline (not as document).
 * Supports .mp4, .gif (sent as gifPlayback), or frame PNGs (auto-converted to mp4 via ffmpeg).
 *
 * @param {string} jid - WhatsApp JID to send to
 * @param {string|string[]} input - File path (mp4/gif) OR array of frame PNG paths
 * @param {object} opts - { caption, fps, gifPlayback, loop }
 * @returns {Promise<boolean>}
 */
export async function sendVideoToJid(jid, input, opts = {}) {
  if (!currentSock) {
    log.warn({ jid }, 'sendVideoToJid: no socket');
    return false;
  }
  ...
  if (Array.isArray(input)) {
    // Convert frame PNGs to MP4 via ffmpeg
    videoPath = await framesToMp4(input, opts);
  } else {
    videoPath = resolveFilePath(input, true);
  }

  const buffer = readFileSync(videoPath);
  const ext = extname(videoPath).toLowerCase();
  const isGif = opts.gifPlayback || ext === '.gif';
  const caption = opts.caption || videoPath.split(/[/\\]/).pop();
  const sizeStr = buffer.length < 1048576
    ? `${(buffer.length / 1024).toFixed(1)}KB`
    : `${(buffer.length / 1048576).toFixed(1)}MB`;

  log.info({ caption, size: sizeStr, gif: isGif }, 'Sending video');
  ...
  const sent = await Promise.race([
    ...
    new Promise((_, reject) => setTimeout(() => reject(new Error('sendVideo timeout (30s)')), 30_000)),
  ]);
  trackSent(sent);
  log.info({ caption }, 'Video sent');

  // Clean up temp file if we generated it
  if (Array.isArray(input) && videoPath.includes('_anim_')) {
...
/**
 * Convert an array of frame PNG paths to an MP4 video using ffmpeg.
 * @param {string[]} framePaths - Absolute paths to frame PNGs (in order)
 * @param {object} opts - { fps: 8, loop: 3 }
 * @returns {string} Path to generated MP4
 */
function framesToMp4(framePaths, opts = {}) {
  const { fps = 8, loop = 3 } = opts;
  const tmpDir = join(config.workspaceDir, 'tmp');
  mkdirSync(tmpDir, { recursive: true });

  // Create a concat file listing frames (repeat for loop count)
  const concatFile = join(tmpDir, `_anim_concat_${Date.now()}.txt`);
  const lines = [];
  for (let i = 0; i < loop; i++) {
    for (const fp of framePaths) {
      const absPath = isAbsolute(fp) ? fp : resolve(config.workspaceDir, fp);
      lines.push(`file '${absPath.replace(/\\/g, '/')}'`);
      lines.push(`duration ${1 / fps}`);
    }
  }
  // ffmpeg concat needs last frame repeated without duration
  const lastFrame = isAbsolute(framePaths.at(-1)) ? framePaths.at(-1) : resolve(config.workspaceDir, framePaths.at(-1));
  lines.push(`file '${lastFrame.replace(/\\/g, '/')}'`);
  writeFileSync(concatFile, lines.join('\n'));

  const outPath = join(tmpDir, `_anim_${Date.now()}.mp4`);
  try {
    execSync(
      `ffmpeg -y -f concat -safe 0 -i "${concatFile}" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -c:v libx264 -pix_fmt yuv420p -preset ultrafast -crf 18 "${outPath}"`,
...
async function sendFileToWhatsApp(sock, jid, filePath, restrictToWorkspace = true) {
  ...
  if (IMAGE_EXTENSIONS.has(ext)) {
    ...
    log.info({ fileName }, 'File sent (video)');
    return sent;
  } else {
    const sent = await sock.sendMessage(jid, {
      document: buffer,
...
export async function startWhatsApp({ queue, botApi } = {}) {
  ...
  let lastOpenTs = 0;         // timestamp of last 'open' — detects flapping (open→close in <10s)
  let reconnectTimer = null;  // pending reconnect timer — prevents overlapping schedules
  let socketGeneration = 0;   // increments on each connectSocket() — stale sockets' events are ignored

  function connectSocket() {
    // Increment generation FIRST — any pending events from the old socket will see a stale gen
    socketGeneration++;

    // Close previous socket before creating a new one — prevents conflict/replaced loops
    if (currentSock) {
      try { currentSock.ev.removeAllListeners('connection.update'); } catch {}
      try { currentSock.end(undefined); } catch {}
      currentSock = null;
    ...
    // Simple in-memory store for sent messages — needed for Baileys retry/re-encryption
    ...
    const sock = makeWASocket({
      ...
      getMessage: async (key) => {
        const msg = sentMsgStore.get(key.id);
        if (msg) return msg;
        return { conversation: '' };
      },
    });

    // Track outgoing messages for retry support
    sock.ev.on('messages.upsert', ({ messages: msgs }) => {
      for (const m of msgs) {
        if (m.key?.fromMe && m.key?.id && m.message) {
          sentMsgStore.set(m.key.id, m.message);
          // Evict old entries to prevent memory leak
          if (sentMsgStore.size > MSG_STORE_MAX) {
    ...
    const myGeneration = socketGeneration; // capture for this socket's lifetime
    sock.ev.on('connection.update', ({ connection, lastDisconnect, qr }) => {
      // Guard: ignore events from stale sockets — prevents ghost close handlers
      // from scheduling competing reconnects that cause 440 conflict/replaced loops
      if (myGeneration !== socketGeneration) {
        log.debug({ myGeneration, currentGeneration: socketGeneration, connection },
          'Ignoring stale socket event');
        return;
      }
      if (qr) {
      ...
      if (connection === 'close') {
        // Suppress WA notifications during reconnect to prevent feedback storm:
        // notify() → sendToGroup() → WA send races with reconnect → 440 → loop
        suppressWaNotify();
        ...
        // 440 = conflict/replaced — another connection replaced us (usually ourselves from a
        // previous reconnect cycle). After 3+ consecutive 440s, jump to 90s backoff to let
        // the ghost connection fully die before we try again.
        if (statusCode === 440 && reconnectCount >= 3) {
          reconnectDelay = Math.max(reconnectDelay, 90000);
          log.warn({ attempt: reconnectCount, nextDelayMs: reconnectDelay },
            '440 conflict loop detected — forcing 90s cooldown to break the cycle');
        }

        const nextDelay = reconnectDelay;
        reconnectDelay = Math.min(reconnectDelay * 2, 120000); // cap at 120s (was 60s)
        ...
        waChannelDisconnect();
        clearTimeout(reconnectTimer);
        reconnectTimer = setTimeout(connectSocket, nextDelay);
      } else if (connection === 'open') {
        // Cancel any pending reconnect timer — we're already connected.
        // Without this, a stale timer can fire and call connectSocket() again,
        // creating a second socket → WhatsApp conflict:replaced → 440 loop.
        clearTimeout(reconnectTimer);
        reconnectTimer = null;

        const now = Date.now();
        const wasFlapping = lastOpenTs > 0 && (now - lastOpenTs) < 30_000;
        lastOpenTs = now;

        if (wasFlapping) {
          // Connection alive < 30s — don't reset backoff, keep escalating
          log.warn({ reconnectDelay, reconnectCount },
            'Connection flapping (open→close < 30s) — keeping backoff pressure');
        } else {
          reconnectDelay = 3000; // reset backoff only if connection was stable
          reconnectCount = 0;
        }
        ...
        // Delay WA channel registration and online notify to let initial sync complete.
        // Sending WA messages immediately on 'open' causes conflict/replaced storms:
        // notify() → sendToGroup() → WA send races with AwaitingInitialSync → 440 → loop
        setTimeout(() => {
          releaseWaNotify(); // Safe to send WA notifications again after sync settles
          waChannelSetSend(sendWhatsAppMessage);
          if (!wasFlapping) {
...
async function handleMessage(sock, msg, botApi) {
  ...
  try {
    ...
    const { reply, claudeMs, filesToSend, videosToSend, dataMessages, shouldClearHistory, costUsd, inputTokens, outputTokens } = await chat(history, onChunk, botContext, { tier, onToolUse });
    ...
    // Send videos from [SEND_VIDEO:] markers
    if (videosToSend && videosToSend.length > 0) {
      log.info({ count: videosToSend.length }, 'Sending videos from SEND_VIDEO markers');
      for (const input of videosToSend) {
        try {
          await sendVideoToJid(sender, input);
        } catch (err) {
          log.error({ err: err.message }, 'Failed to send video');
          const errSent = await sock.sendMessage(sender, {
            text: `Could not send video: ${err.message}`,
          });
          trackSent(errSent);
        }
      }
...
async function handleMediaMessage(sock, msg, sender, contentType) {
  ...
  try {
    ...
    const { reply, claudeMs, filesToSend, videosToSend: mediaVideos, costUsd, inputTokens, outputTokens } = await chat(history, onChunk, botContext, { tier });
    ...
    // Send videos from [SEND_VIDEO:] markers (media handler)
    if (mediaVideos && mediaVideos.length > 0) {
      for (const input of mediaVideos) {
        try { await sendVideoToJid(sender, input); } catch (err) {
          log.error({ err: err.message }, 'Failed to send video (media handler)');
        }
      }
    }
