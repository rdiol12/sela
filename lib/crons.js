...
/** Estimate minimum interval in minutes from a cron expression. Returns Infinity if cannot parse. */
function estimateMinInterval(schedule) {
  const parts = schedule.trim().split(/\s+/);
  if (parts.length < 5) return Infinity; // not standard cron
  const minutePart = parts[0];
  const hourPart = parts[1];
  // If minute is * and hour is *, fires every minute
  if (minutePart === '*' && hourPart === '*') return 1;
  // If minute is */N, fires every N minutes
  const stepMatch = minutePart.match(/^\*\/(\d+)$/);
  if (stepMatch) return parseInt(stepMatch[1], 10);
  // If minute is * (but hour is not *), fires every minute within that hour
  if (minutePart === '*') return 1;
  // If minute is a list like "0,5,10", estimate from gap
  if (minutePart.includes(',')) {
    const vals = minutePart.split(',').map(Number).filter(n => !isNaN(n)).sort((a,b) => a - b);
    if (vals.length >= 2) {
      let minGap = 60;
  ...
  // If minute is a range like "0-5", fires every minute in that range
  if (minutePart.includes('-') && !minutePart.includes('/')) return 1;
  // Single number = fires once per hour at minimum
  return 60;
}
...
export function load() {
  try {
    ...
    // Boot-time safety sweep: auto-disable any sub-5min crons that snuck into DB
    // (defense against direct SQL inserts or guard bypasses — a * * * * * cron cost $28 on Mar 8 2026)
    for (const job of jobs) {
      if (job.enabled) {
        const minInt = estimateMinInterval(job.schedule);
        if (minInt < 5) {
          log.warn({ id: job.id, name: job.name, schedule: job.schedule, minInterval: minInt },
            'Boot sweep: disabling dangerously frequent cron');
          job.enabled = false;
          cronToDb(job);
        }
      }
...
function scheduleJob(job) {
  ...
  // Runtime guard: refuse to schedule anything that fires more often than every 5 minutes
  const minInt = estimateMinInterval(job.schedule);
  if (minInt < 5) {
    log.warn({ job: job.name, id: job.id, schedule: job.schedule, minInterval: minInt }, 'Refusing to schedule sub-5min cron — auto-disabling');
    job.enabled = false;
    cronToDb(job);
    return;
  }
...
export function addCron(name, schedule, prompt, tz, delivery = 'announce', model = null) {
  ...
  // Block dangerously frequent schedules (every minute burns ~$0.50/min)
  // Estimate minimum interval from the cron expression
  const minIntervalMinutes = estimateMinInterval(schedule);
  if (minIntervalMinutes < 5) {
    throw new Error(`Schedule "${schedule}" fires every ~${minIntervalMinutes} minute(s) — too expensive ($0.50/fire). Use at least a 5-minute interval (e.g. "*/5 * * * *").`);
  }
