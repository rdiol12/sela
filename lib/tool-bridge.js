...
// 5. Generate 3D Asset (Shattered Crown — calls Blender MCP via asset-pipeline)
registerTool({
  name: 'generate_asset',
  description: 'Generate next pending 3D asset from asset-manifest.json via Blender MCP. No params needed — picks the next pending asset automatically. Returns { success, assetId, outputPath } or { success: false, error }.',
  async execute() {
    const { generateOneAsset } = await import('./asset-pipeline.js');
    return generateOneAsset();
  },
}, { rateLimit: 60000 }); // 1 min rate limit — asset generation is slow

// 6. Get asset pipeline progress
registerTool({
  name: 'asset_progress',
  ...
  async execute() {
    const { getStatusReport } = await import('./asset-pipeline.js');
