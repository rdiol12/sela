...
registerTool({
  ...
  async execute(params) {
    ...
    if (!resolved.startsWith(selaRoot + (resolved === selaRoot ? '' : sep))) {
...
registerTool({
  ...
  async execute(params) {
    ...
    if (!resolved.startsWith(config.workspaceDir + sep) && resolved !== config.workspaceDir) {
      throw new Error('Access denied');
    }
...
// NOTE: generate_asset, asset_progress → moved to modules/asset-pipeline/index.js
// NOTE: generate_audio, audio_progress → moved to modules/audio-pipeline/index.js
// NOTE: unreal_run_command, unreal_get_scene → moved to modules/unreal/index.js
