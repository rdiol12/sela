...
import { getGoalsContext, getGoalsContextCompact, listGoals } from './goals.js';
...
### Blender aesthetics (lib/blender-knowledge.js)
When executing execute_blender_code, inject patterns from lib/blender-knowledge.js:
- getBlenderPreamble() — helper functions (clear_scene, apply_smooth, add_subsurf, add_bevel)
- MATERIALS: make_pbr, make_stone, make_metal, make_emissive, make_wood
- LIGHTING: setup_studio_lights(), setup_fantasy_lights(), setup_outdoor_lights()
- MODIFIERS: smoothRounded, solidify, decimate
Always: apply smooth shading + assign a material to every mesh you create.`;
...
export function assemblePrompt(tier = 'standard', opts = {}) {
  ...
  switch (tier) {
    ...
    case 'standard': {
      ...
      // Progressive disclosure: compact goals in standard tier
      try {
        const compactGoals = getGoalsContextCompact();
        if (compactGoals) parts.push(`\n## Active Goals\n${compactGoals}`);
      } catch {}
