 ...
 *
 * Cost controls: Haiku for all cycles except high/critical signals. $50/day hard cost cap (DAILY_COST_LIMIT env, enforced in runAgentCycle + runProjectCycle). Backoff after 10 consecutive spawns.
 */

import { statSync, mkdirSync, writeFileSync, readdirSync, readFileSync, existsSync, appendFileSync } from 'fs';
...
import { listGoals, getStaleGoals, getUpcomingDeadlines, addGoal, updateGoal, completeMilestone, getGoalsContext, getGoalsContextCompact, getGoal, getJsonMtime, importJsonChanges, load as reloadGoals } from './goals.js';
import { listCrons } from './crons.js';
import { emit as wsEmit } from './ws-events.js';
import config from './config.js';
import { getCostOverview, rollupOldCosts, recordCostEntry, getTodayCost } from './cost-analytics.js';
...
import { getLowEngagementCrons, recordBotReply, captureUserReaction } from './outcome-tracker.js';
import { getLearningContext } from './agent-learning.js';

import { logError, insertCost } from './db.js';
...
import { buildProjectInjection, detectPhaseTransitions, recordProjectCycleSummary } from './project-cycle-injector.js';
...
import { getModuleBriefBuilders, getModuleSonnetSignalTypes, getModuleStateKeyMaps, checkModuleUrgentWork, getModuleMessageCategories, getProjectProcessConfigs } from './module-loader.js';
import { listAgentCycleProjects } from './projects.js';
import { getBehaviorModifiers } from './behavior-adaptor.js';
import { nimChat } from './nim-client.js';
import { addHypothesis, addEvidence, conclude as concludeHypothesis, pruneOld as pruneReasoningJournal } from './reasoning-journal.js';
...
// formatLearningContext moved to lazy-context.js for progressive disclosure
import { runIfDue as runPainPointAnalysis } from './pain-point-analyzer.js';
import { buildGoalsContextLazy, buildModuleContextLazy, buildPatternInsightsLazy, buildLearningContextLazy, buildReasoningContextLazy, registerContextTools, recordSavings } from './lazy-context.js';

const log = createLogger('agent-loop');

/** Merge module-defined + user-defined (dynamic) project configs. */
function getAllProjectConfigs() {
  const moduleConfigs = getProjectProcessConfigs();
  const moduleSlugs = new Set(moduleConfigs.map(pc => pc.project || pc.name));
  try {
    const dynamic = listAgentCycleProjects().filter(dc => !moduleSlugs.has(dc.project));
    return [...moduleConfigs, ...dynamic];
  } catch {
...
function pickSignals(signals, state) {
  ...
  // Pipeline signals (asset/audio generation) get their own slot — never starved by followups
  const PIPELINE_TYPES = new Set(['asset_generation', 'asset_regeneration', 'audio_generation']);
  const pipelineSignal = escalated.find(s => PIPELINE_TYPES.has(s.type));

  for (const s of escalated) {
    if (picked.length >= 2) break;
    const needsSonnet = SONNET_SIGNAL_TYPES.has(s.type);
    if (needsSonnet && sonnetCount >= 2) continue; // allow 2 Sonnet signals (was 1)
  ...
  // Guarantee pipeline signal gets a slot — AFTER diversity swap so it can't be overwritten
  if (pipelineSignal && !picked.some(p => PIPELINE_TYPES.has(p.type))) {
    if (picked.length >= 2) picked[1] = pipelineSignal;
    else picked.push(pipelineSignal);
  }

  // Project round-robin: if both signals target the same project, swap slot 2
  // for a signal from a different project to prevent starvation
  if (picked.length === 2) {
    const proj0 = getSignalProject(picked[0]);
    const proj1 = getSignalProject(picked[1]);
    if (proj0 && proj1 && proj0 === proj1) {
      const alt = escalated.find(s => !picked.includes(s) && getSignalProject(s) !== proj0);
      if (alt) picked[1] = alt;
    }
  }
...
- **Call pipeline tools** (generate_asset, generate_asset_batch, generate_audio, generate_audio_batch, regenerate_asset) — these are pre-built automation, NOT code work
...
IMPORTANT: If a signal brief says "ACTION REQUIRED — EXECUTE NOW", you MUST call the tool immediately. Pipeline tools are safe to call.`;
...
let running = false;
let stopped = false;
let mainPaused = false;                  // pause main cycle without stopping loop
const projectPaused = new Map();         // project → boolean
...
let coreProc = null;                  // PersistentProcess for non-project signals (uses mcp-config-core)
const projectProcs = new Map();       // project name → PersistentProcess
let agentSessionCycles = 0;
let agentSessionTokens = 0;
const projectSessionCycles = new Map(); // project → cycle count since last reset
const projectSessionTokens = new Map(); // project → token count since last reset

// Classify a signal into a project based on module prefix or goal data
function getSignalProject(signal) {
  const cats = getModuleMessageCategories();
  for (const [prefix, category] of Object.entries(cats)) {
    if (signal.type.startsWith(prefix)) return category;
  }
  if (signal.data?.goalId) {
    try {
      const goal = getGoal(signal.data.goalId);
      if (goal?.project) return goal.project;
    } catch {}
  }
  return null;
}

function getProcessForSignals(signals) {
  const projects = signals.map(s => getSignalProject(s)).filter(Boolean);
  if (projects.length === 0) return coreProc;
  // Pick the most common project among picked signals
  const counts = {};
  for (const p of projects) counts[p] = (counts[p] || 0) + 1;
  const dominant = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
  return projectProcs.get(dominant) || coreProc;
}

function resetAgentSession(reason) {
  log.info({ reason, cycles: agentSessionCycles, tokens: agentSessionTokens }, 'Agent session reset');
  coreProc?.respawnForCompression(randomUUID(), AGENT_SESSION_SYSTEM_PROMPT);
  agentSessionCycles = 0;
  agentSessionTokens = 0;
}

/**
 * Reset the proc actually being used (project or core) when session limits are hit.
 * Fixes the bug where resetAgentSession() only reset coreProc even when a project proc was in use.
 */
function resetProcIfNeeded(proc, ctx) {
  const needsReset = agentSessionTokens > SESSION_TOKEN_LIMIT || agentSessionCycles >= SESSION_CYCLE_LIMIT;
  if (!needsReset) return;
  const reason = agentSessionTokens > SESSION_TOKEN_LIMIT ? 'token_limit' : 'cycle_limit';
  log.info({ reason, ctx, cycles: agentSessionCycles, tokens: agentSessionTokens, isCore: proc === coreProc }, 'Proc session reset');
  proc?.respawnForCompression(randomUUID(), AGENT_SESSION_SYSTEM_PROMPT);
  // Also reset coreProc if it's a different proc (keep both fresh)
  if (proc !== coreProc) coreProc?.respawnForCompression(randomUUID(), AGENT_SESSION_SYSTEM_PROMPT);
  agentSessionCycles = 0;
  agentSessionTokens = 0;
}

/** Safety valve: if cacheRead ballooned (> 400K tokens), reset before next cycle to prevent OOM. */
function checkCacheBloom(proc, cacheRead, ctx) {
  const CACHE_BLOOM_LIMIT = 400_000;
  if ((cacheRead || 0) > CACHE_BLOOM_LIMIT) {
    log.warn({ cacheRead, ctx, limit: CACHE_BLOOM_LIMIT }, 'Cache bloom detected — scheduling proc reset before next cycle');
    // Mark for reset: set counters past the limit so next call to resetProcIfNeeded fires
    agentSessionTokens = SESSION_TOKEN_LIMIT + 1;
  }
}

function resetProjectSession(project, reason) {
  const proc = projectProcs.get(project);
  if (!proc) return;
  const cycles = projectSessionCycles.get(project) || 0;
  const tokens = projectSessionTokens.get(project) || 0;
  log.info({ project, reason, cycles, tokens }, 'Project session reset');
  proc.respawnForCompression(randomUUID(), AGENT_SESSION_SYSTEM_PROMPT);
  projectSessionCycles.set(project, 0);
  projectSessionTokens.set(project, 0);
}
...
const QMD_MAX_DURATION_MS = 30 * 60_000;  // kill if running > 30 min

// QMD sync tracking — exposed via getAgentLoopDetail()
const qmdSync = { running: false, startedAt: 0, lastResult: null, lastDurationMs: 0, pid: null, failures: 0 };
...
function qmdSyncIfDue(state) {
  ...
  // Kill hung QMD process if still running past max duration
  if (qmdSync.running && qmdSync.pid && (Date.now() - qmdSync.startedAt) > QMD_MAX_DURATION_MS) {
    log.warn({ pid: qmdSync.pid, durationMs: Date.now() - qmdSync.startedAt }, 'QMD sync hung — killing');
    try { process.kill(qmdSync.pid); } catch {}
    qmdSync.running = false;
    qmdSync.lastResult = 'killed (hung)';
    qmdSync.failures++;
    return;
  }

  // Don't start a new sync if one is already running
  if (qmdSync.running) return;
  ...
  // Async sync — tracked but never blocks event loop
  state.lastQmdSyncAt = Date.now();
  qmdSync.running = true;
  qmdSync.startedAt = Date.now();
  qmdSync.lastResult = null;

  const qmdBin = join(process.cwd(), 'node_modules', '.bin', process.platform === 'win32' ? 'qmd.cmd' : 'qmd');
  const proc = exec(`"${qmdBin}" update`, { timeout: 15 * 60_000, windowsHide: true }, (err) => {
    if (err) {
      log.warn({ err: err.message }, 'QMD update failed');
      qmdSync.running = false;
      qmdSync.lastResult = 'update failed: ' + err.message;
      qmdSync.lastDurationMs = Date.now() - qmdSync.startedAt;
      qmdSync.failures++;
      return;
    }
    const proc2 = exec(`"${qmdBin}" embed`, { timeout: 20 * 60_000, windowsHide: true }, (err2) => {
      qmdSync.running = false;
      qmdSync.lastDurationMs = Date.now() - qmdSync.startedAt;
      qmdSync.pid = null;
      if (err2) {
        log.warn({ err: err2.message }, 'QMD embed failed');
        qmdSync.lastResult = 'embed failed: ' + err2.message;
        qmdSync.failures++;
      } else {
        log.info({ durationMs: qmdSync.lastDurationMs }, 'QMD update+embed completed');
        qmdSync.lastResult = 'ok';
        qmdSync.failures = 0;
      }
    });
    qmdSync.pid = proc2.pid;
  });
  qmdSync.pid = proc.pid;
}

export function getQmdSyncStatus() {
  return { ...qmdSync, elapsedMs: qmdSync.running ? Date.now() - qmdSync.startedAt : 0 };
}
...
function collectSignals(state) {
  ...
  // Reload goals from SQLite to prevent stale in-memory state causing phantom signals
  try { reloadGoals(); } catch {}
  try {
  ...
  // Module signals now handle: asset-pipeline, audio-pipeline, unreal, hattrick
  // (detectModuleSignals already called above — covers all module signal detectors)
  ...
  // 15. Strip signals that belong to projects with independent cycles
  // These projects collect their own signals in runProjectCycle — no duplication.
  const independentProjects = getAllProjectConfigs().filter(pc => pc.cycleInterval);
  if (independentProjects.length > 0) {
    const prefixes = independentProjects.map(pc => pc.signalPrefix).filter(Boolean);
    const projectNames = new Set(independentProjects.map(pc => pc.project || pc.name));
    // Pre-compute project goals for followup topic matching (avoid repeated listGoals in filter)
    let projectGoalTitles = [];
    try {
      projectGoalTitles = listGoals({ status: ['active', 'in_progress'] })
        .filter(g => g.project && projectNames.has(g.project))
        .flatMap(g => [
          g.title.toLowerCase(),
          ...(g.milestones || []).map(m => m.title?.toLowerCase()).filter(Boolean),
        ]);
    } catch {}

    return signals.filter(s => {
      // Remove signals matching a project's signal prefix (e.g. maple_*, hattrick_*)
      if (prefixes.some(p => s.type.startsWith(p))) return false;
      // Remove goal_work signals whose goal belongs to an independent project
      if (s.type === 'goal_work' && s.data?.goalId) {
        try {
          const goal = getGoal(s.data.goalId);
          if (goal?.project && projectNames.has(goal.project)) return false;
        } catch {}
      }
      // Remove stale_goal signals for independent projects
      if (s.type === 'stale_goal' && s.data?.goalId) {
        try {
          const goal = getGoal(s.data.goalId);
          if (goal?.project && projectNames.has(goal.project)) return false;
        } catch {}
      }
      // Remove followups that reference goals in independent projects
      if (s.type === 'followup') {
        // By explicit goalId
        if (s.data?.goalId) {
          try {
        ...
        // By topic fuzzy-match against project goal titles (catches followups without goalId)
        const topic = (s.data?.topic || '').toLowerCase();
        if (topic && projectGoalTitles.some(t => topic.includes(t))) return false;
      }
      return true;
    });
  }
...
// ─── Project-Scoped Signal Collection ────────────────────────────────────────
// Each project with an independent cycle collects its OWN signals — no mixing.

async function collectProjectSignals(pc, projectState) {
  const signals = [];

  // 1. Module's own signal detector
  if (typeof pc.detectSignals === 'function') {
    try {
      const modSignals = await pc.detectSignals(projectState);
      if (Array.isArray(modSignals)) signals.push(...modSignals);
    } catch (err) {
      log.warn({ project: pc.project, err: err.message }, 'Project signal detector failed');
    }
  }

  // 2. Project-scoped goal_work — only goals belonging to this project
  try { reloadGoals(); } catch {}
  try {
    const prioOrder = { critical: 0, high: 1, medium: 2, normal: 3, low: 4 };
    const workable = listGoals({ status: ['active', 'in_progress'] })
      .filter(g => g.project === pc.project)
      .filter(g => g.milestones?.some(m => m.status === 'pending'))
      .sort((a, b) => (prioOrder[a.priority] ?? 3) - (prioOrder[b.priority] ?? 3));
    for (const g of workable.slice(0, 3)) {
      const nextMs = g.milestones.find(m => m.status === 'pending');
    ...
    log.warn({ project: pc.project, err: err.message }, 'Project goal_work collection failed');
  }

  // 3. Project-scoped followups from project state
  try {
    const activeGoals = listGoals({ status: ['active', 'in_progress'] }).filter(g => g.project === pc.project);
    for (const f of (projectState.pendingFollowups || [])) {
      const topic = f.topic || '';
      const matched = (f.goalId && activeGoals.find(g => g.id === f.goalId))
        || activeGoals.find(g =>
          topic.toLowerCase().includes(g.title.toLowerCase()) ||
    ...
    log.warn({ project: pc.project, err: err.message }, 'Project followup collection failed');
  }

  // 4. Project-scoped stale goals
  try {
    const stale = getStaleGoals(48).filter(g => g.project === pc.project);
    for (const g of stale) {
      const hoursSince = Math.round((Date.now() - g.updatedAt) / 3600_000);
      signals.push({
        type: 'stale_goal',
    ...
    log.warn({ project: pc.project, err: err.message }, 'Project stale goals collection failed');
  }
...
function buildAgentPrompt(signals) {
  ...
  // Progressive disclosure v2: lazy context loading based on signal relevance
  const patternInsights = buildPatternInsightsLazy(signals);
  ...
  const _sonnetTypes = getSonnetSignalTypes();
  const useSonnet = signals.some(s => s.urgency === 'high' || s.urgency === 'critical') ||
    ...
    signals.some(s => _sonnetTypes.has(s.type));

  // Auto-coder: inject pending milestone brief for Sonnet cycles only
  // Skip when pipeline signals are active — pipeline tools do the work, not code writing
  const PIPELINE_SIGNAL_TYPES = new Set(['asset_generation', 'asset_regeneration', 'audio_generation']);
  const hasPipelineSignal = signals.some(s => PIPELINE_SIGNAL_TYPES.has(s.type));
  let milestoneBrief = '';
  if (useSonnet && !hasPipelineSignal) {
    try {
      // Filter out goals belonging to projects with independent cycles — they have their own pickMilestone
      const independentNames = new Set(getAllProjectConfigs().filter(pc => pc.cycleInterval).map(pc => pc.project || pc.name));
      const mainGoals = listGoals().filter(g => !g.project || !independentNames.has(g.project));
      const picked = pickMilestone(mainGoals);
  ...
  // Project injection removed from main cycle — projects with independent cycles
  // handle their own context via buildProjectInjection in runProjectCycle.

  // Progressive disclosure v2: signal-aware goals context (exclude independent-project goals)
  const independentProjectNames = new Set(getAllProjectConfigs().filter(pc => pc.cycleInterval).map(pc => pc.project || pc.name));
  const goalsCtx = buildGoalsContextLazy(signals, useSonnet, { excludeProjects: independentProjectNames });
  ...
  // Progressive disclosure v2: module context only when signal-relevant
  for (const ctx of buildModuleContextLazy(signals)) {
  ...
  for (const sig of signals) {
    ...
    if (builder) {
      try {
        const b = builder(sig);
        if (b) {
          // Brief builders return { title, content, reasoning } — format as string
          if (typeof b === 'string') contextParts.push(b);
          else if (b.title || b.content) contextParts.push(`## ${b.title || sig.type}\n${b.content || ''}${b.reasoning ? `\n_Reasoning: ${b.reasoning}_` : ''}`);
          else contextParts.push(String(b));
        }
      } catch {}
    }
  }

  // Progressive disclosure v2: learning + reasoning gated by signal relevance
  try {
    const learningCtx = buildLearningContextLazy(signals, useSonnet);
    if (learningCtx) contextParts.push(learningCtx);
  } catch {}
  try {
    const reasoningCtx = buildReasoningContextLazy(signals, useSonnet);
    if (reasoningCtx) contextParts.push(reasoningCtx);
  } catch {}

  const contextBlock = `<context>\n${contextParts.join('\n\n')}\n</context>`;

  // Progressive disclosure v2: measure context size for ms_3 tracking
  const promptTokenEstimate = Math.round(contextBlock.length / 4); // ~4 chars per token
  const fullContextEstimate = 7500; // baseline average before lazy-context
  const saved = Math.max(0, fullContextEstimate - promptTokenEstimate);
  if (saved > 0) recordSavings(saved);
  log.debug({ promptTokens: promptTokenEstimate, savedTokens: saved, model: useSonnet ? 'sonnet' : 'haiku' }, 'Prompt size (progressive disclosure v2)');
...
- CYCLE_DONE only if genuinely nothing to do.${useSonnet ? '\n\n' + PROJECT_PATHS_HINT : ''}${milestoneBrief}`;
}
...
function buildNimPrompt(signals) {
  ...
  // Progressive disclosure: Nim uses compact goals (Haiku tier)
  const goalsCtx = getGoalsContextCompact();
...
function buildNimReflectionPrompt() {
  ...
  const dateStr = now.toISOString().slice(0, 10);
  const quiet = isQuietHours();
  // Progressive disclosure: reflection uses compact goals
  const goalsCtx = getGoalsContextCompact();
...
// Simpler than buildAgentPrompt — focused on one project's signals + goals.

function buildProjectPrompt(signals, pc, state = {}) {
  const now = israelNow();
  const timeStr = now.toLocaleTimeString('en-IL', { hour: '2-digit', minute: '2-digit', hour12: false });
  const dateStr = now.toISOString().slice(0, 10);
  const quiet = isQuietHours();
  ...
  const useSonnet = signals.some(s => s.urgency === 'high' || s.urgency === 'critical') ||
    signals.some(s => s.type === 'goal_work' && CODE_KEYWORDS.test(s.data?.nextMilestone || '')) ||
    signals.some(s => s.type === 'followup' && CODE_KEYWORDS.test(s.data?.topic || '')) ||
    signals.some(s => pc.sonnetSignalTypes?.has(s.type));

  // Project-scoped goals context (compact to keep prompt size manageable)
  const goalsCtx = getGoalsContextCompact(pc.project);

  const contextParts = [`## Current context:\n${dateStr} ${timeStr} Israel${quiet ? ' (QUIET HOURS)' : ''}\nProject: ${pc.project}`];
  contextParts.push(`## Detected signals:\n${signalBlock}`);
  if (goalsCtx) contextParts.push(`## Active goals (${pc.project}):\n${goalsCtx}`);

  // Module brief builders for this project's signal types
  for (const sig of signals) {
    const builder = pc.briefBuilders[sig.type];
    if (builder) {
      try {
        const b = builder(sig);
        if (b) {
  ...
  // Cross-cycle memory: recent actions
  try {
    const recentBlock = getRecentActionsBlock();
    if (recentBlock) contextParts.push(recentBlock);
  } catch {}

  // Generic project injection: loads project-specific context (rules, council, protocol phases)
  // Config-driven from data/project-injections/{project}.json — works for any project
  if (pc?.project) {
    try {
      const followups = (state?.pendingFollowups || []);
      const projectCtx = buildProjectInjection(pc.project, signals, followups);
      if (projectCtx) contextParts.push(projectCtx);
    } catch (err) {
      log.warn({ project: pc.project, err: err.message }, 'Project injection failed');
    }
  }

  // Auto-coder milestone brief for Sonnet cycles
  let milestoneBrief = '';
  if (useSonnet) {
    try {
      const projectGoals = listGoals({ status: ['active', 'in_progress'] }).filter(g => g.project === pc.project);
      const picked = pickMilestone(projectGoals);
      if (picked) milestoneBrief = '\n\n' + buildMilestoneBrief(picked.goal, picked.milestone);
    } catch {}
  }
...
AGENT_CYCLE [PROJECT: ${pc.project}]:${quiet ? ' (QUIET HOURS — do NOT send WhatsApp messages)' : ''}
You are the user's autonomous agent. This cycle is dedicated to the **${pc.project}** project. Focus all work on this project's goals and signals.

## Instructions — Investigate → Decide → Act → Verify:
1. **Investigate**: Use tools to understand each signal deeply. Don't just glance — dig in.
2. **Decide**: What needs action NOW vs what can wait? Bias toward action.
3. **Act**: Update goals, advance milestones, create content, fix issues. Do multiple things if needed.
4. **Verify**: Confirm your actions worked. Re-check after changing state.

After handling signals: pick an active goal and advance it.
${useSonnet ? '\n' + SONNET_CODE_RULES + '\n' : '\n' + HAIKU_RESTRICTIONS + '\n'}
${buildOutputTags(quiet)}
...
async function runAgentCycle() {
  ...
  if (stopped) return;
  if (mainPaused) {
    log.info('Agent cycle skipped — main cycle paused');
    scheduleNext(INTERVAL_MS);
    return;
  }

  // ── Hard daily cost cap ──
  const DAILY_HARD_CAP = parseFloat(process.env.DAILY_COST_LIMIT || '50');
  const todayCost = getTodayCost();
  if (todayCost >= DAILY_HARD_CAP) {
    log.warn({ todayCost: todayCost.toFixed(2), cap: DAILY_HARD_CAP }, 'Agent cycle BLOCKED — daily cost cap exceeded');
    return; // Don't set running=true, just skip entirely
  }
  ...
  try {
    ...
    log.info({ rawCount: rawSignals.length, rawTypes: rawSignals.map(s => s.type) }, 'Agent cycle: raw signals collected');
    const cooled = filterCooldowns(rawSignals, state);
    log.info({ cooledCount: cooled.length, cooledTypes: cooled.map(s => s.type) }, 'Agent cycle: after cooldown filter');
    ...
    const hasSonnetSignalType = signals.some(s => getSonnetSignalTypes().has(s.type));
    // Sonnet for: high/critical signals, code goal work, code followups, module-registered Sonnet types (asset/audio pipelines).
    ...
    const useSonnet = !isReflectionCycle && !sonnetCoolingDown && (hasHighOrCriticalSignal || hasCodeGoalWork || hasCodeFollowup || hasSonnetSignalType);
    ...
    try {
      ...
      if (freeBackend) {
        ...
        try {
          ...
          const usedProcFb = getProcessForSignals(signals || []);
          resetProcIfNeeded(usedProcFb, 'fallback');
          const result = await usedProcFb.send(prompt);
          reply = result.text;
          ...
          checkCacheBloom(usedProcFb, cacheRead, 'fallback');
        }
      } else {
        // Sonnet cycle or all free backends unavailable — use persistent Claude process
        const usedProc = getProcessForSignals(signals || []);
        resetProcIfNeeded(usedProc, 'pre-cycle');
        const result = await usedProc.send(prompt);
        reply = result.text;
        costUsd = result.costUsd || 0;
        inputTokens = result.inputTokens;
        outputTokens = result.outputTokens;
        ...
        agentSessionCycles++;
        agentSessionTokens += (inputTokens || 0) + (outputTokens || 0);
        checkCacheBloom(usedProc, cacheRead, 'post-cycle');
      }
    ...
    // If goals.json was modified externally (by CLI agent, user, or cron), import changes
    // Always check — not just claude cycles — to prevent stale in-memory goal state
    try {
      const goalsJsonMtimeAfter = getJsonMtime();
      if (goalsJsonMtimeAfter > goalsJsonMtimeBefore) {
        const imported = importJsonChanges();
        if (imported > 0) log.info({ imported }, 'Agent cycle: imported direct goals.json edits');
      }
    } catch (err) {
    ...
    // Persist cost to JSONL + SQLite (was missing — persistent process costs went unrecorded)
    if (costUsd > 0 && !config.costTrackingDisabled) {
      const costEntry = {
        type: 'persistent', model: activeBackendModel,
        inputTokens: inputTokens || 0, outputTokens: outputTokens || 0,
        cacheRead: cacheRead || 0, costUsd, durationMs: 0,
        cronId: null, sessionId: coreProc?.sessionId || null,
      };
      try { appendFileSync(config.costsFile, JSON.stringify({ ...costEntry, ts: Date.now() }) + '\n'); } catch (e) { log.warn({ err: e.message }, 'Cost JSONL write failed'); }
      try { insertCost({ ...costEntry, ts: Date.now() }); } catch (e) { log.warn({ err: e.message }, 'Cost DB write failed'); }
      try { recordCostEntry(costUsd); } catch (e) { log.warn({ err: e.message }, 'Cost analytics record failed'); }
      log.debug({ type: 'persistent', costUsd, model: activeBackendModel }, 'Cost entry recorded');
    }
    ...
    // Filter out followups that belong to independent project cycles — they store their own
    const independentProjectNames = new Set(
      getAllProjectConfigs().filter(pc => pc.cycleInterval).map(pc => pc.project || pc.name)
    );
    let mainFollowups = parsed.followups;
    if (independentProjectNames.size > 0) {
      const projGoals = listGoals({ status: ['active', 'in_progress'] })
        .filter(g => g.project && independentProjectNames.has(g.project));
      const projGoalIds = new Set(projGoals.map(g => g.id));
      const projTitles = projGoals.flatMap(g => [
        g.title.toLowerCase(),
        ...(g.milestones || []).map(m => m.title?.toLowerCase()).filter(Boolean),
      ]);
      mainFollowups = parsed.followups.filter(f => {
        if (f.goalId && projGoalIds.has(f.goalId)) return false;
        const topic = (f.topic || '').toLowerCase();
        if (topic && projTitles.some(t => topic.includes(t))) return false;
        return true;
      });
    }
    state.pendingFollowups = mainFollowups.slice(0, MAX_FOLLOWUPS);
    ...
    if (parsed.actionsTaken.length > 0) {
      ...
      wsEmit('agent:cycle:actions', { actions: parsed.actionsTaken, replyFile, cycleCount: state.cycleCount });
      recordEvent(state, 'agent:cycle:actions', { actions: parsed.actionsTaken, replyFile, cycleCount: state.cycleCount });
    ...
    recordEvent(state, 'agent:cycle:complete', {
      ...
      followupCount: parsed.followups.length, replyFile, cycleCount: state.cycleCount,
    });
    saveState(state);
    wsEmit('agent:cycle:complete', {
      ...
      replyFile, cycleCount: state.cycleCount,
    });
...
// ─── Independent Project Cycles ─────────────────────────────────────────────
// Each project with cycleInterval gets its own timer, state, and signal pipeline.

const projectTimers = new Map();         // project → setTimeout handle
const projectNextCycleAt = new Map();    // project → timestamp
const projectRunning = new Map();        // project → boolean
const projectCycleCounts = new Map();    // project → number (for logging)
const projectLiveEvents = new Map();     // project → [{event, ts, data}]
const MAX_PROJECT_EVENTS = 30;

function recordProjectEvent(project, event, data) {
  const entry = { event, ts: Date.now(), data: { ...data, project } };
  if (!projectLiveEvents.has(project)) projectLiveEvents.set(project, []);
  const buf = projectLiveEvents.get(project);
  buf.push(entry);
  if (buf.length > MAX_PROJECT_EVENTS) buf.splice(0, buf.length - MAX_PROJECT_EVENTS);
  // Also push to global liveEvents so main /agent page sees them
  liveEvents.push(entry);
  if (liveEvents.length > MAX_RECENT_EVENTS) liveEvents = liveEvents.slice(-MAX_RECENT_EVENTS);
  // Persist project events so they survive restarts
  try {
    const raw = getState(`agent-loop:${project}`) || {};
    raw.recentEvents = buf.slice(-MAX_PROJECT_EVENTS);
    setState(`agent-loop:${project}`, raw);
  } catch {}
}
...
function loadProjectState(project) {
  ...
  // Hydrate in-memory event buffer from persisted state
  if (state.recentEvents.length > 0 && !projectLiveEvents.has(project)) {
    projectLiveEvents.set(project, [...state.recentEvents]);
  }
  return state;
}

function saveProjectState(project, state) {
  setState(`agent-loop:${project}`, state);
}

async function runProjectCycle(pc) {
  const project = pc.project || pc.name;
  if (projectRunning.get(project)) {
    log.warn({ project }, 'Project cycle skipped — previous still running');
    return;
  }
  if (stopped) return;
  if (projectPaused.get(project)) {
    log.info({ project }, 'Project cycle skipped — paused');
    scheduleProjectNext(project, pc, pc.cycleInterval);
    return;
  }

  // ── Hard daily cost cap (shared with main cycle) ──
  const projDailyCap = parseFloat(process.env.DAILY_COST_LIMIT || '50');
  const projTodayCost = getTodayCost();
  log.debug({ project, todayCost: projTodayCost.toFixed(2), cap: projDailyCap, pid: process.pid }, 'Project cycle cost check');
  if (projTodayCost >= projDailyCap) {
    log.warn({ project, todayCost: projTodayCost.toFixed(2), cap: projDailyCap }, 'Project cycle BLOCKED — daily cost cap exceeded');
    // Also stop future scheduling — no point scheduling more cycles today
    return;
  }

  projectRunning.set(project, true);
  const state = loadProjectState(project);
  state.cycleCount++;
  projectCycleCounts.set(project, state.cycleCount);

  try {
    log.info({ project, cycleCount: state.cycleCount }, 'Project cycle: starting');
    recordProjectEvent(project, 'project:cycle:start', { cycleCount: state.cycleCount });
    wsEmit('project:cycle:start', { project, cycleCount: state.cycleCount });

    // 1. Collect project-scoped signals
    const rawSignals = await collectProjectSignals(pc, state);
    log.info({ project, rawCount: rawSignals.length, types: rawSignals.map(s => s.type) }, 'Project cycle: raw signals');
    const cooled = filterCooldowns(rawSignals, state);
    const signals = pickSignals(cooled, state);
    stampCooldowns(signals, state);
    state.lastSignals = signals.map(s => ({ type: s.type, urgency: s.urgency, summary: s.summary }));
    ...
    if (signals.length === 0) {
      log.info({ project }, 'Project cycle: no signals, skipping');
      recordProjectEvent(project, 'project:cycle:skip', { reason: 'no_signals' });
      state.consecutiveSpawns = 0;
      saveProjectState(project, state);
      return;
    }

    log.info({ project, signals: signals.length, types: signals.map(s => s.type) }, 'Project cycle: signals detected, entering Phase 2');

    // Backoff guard
    if (state.consecutiveSpawns >= BACKOFF_THRESHOLD) {
      log.warn({ project, consecutiveSpawns: state.consecutiveSpawns }, 'Project cycle: backoff');
      recordProjectEvent(project, 'project:cycle:skip', { reason: 'backoff' });
      state.consecutiveSpawns = 0;
      saveProjectState(project, state);
      return;
    }
    ...
    // 2. Build prompt
    const { prompt, useSonnet } = buildProjectPrompt(signals, pc, state);
    const model = useSonnet ? (pc.model || SONNET_MODEL) : ROUTINE_MODEL;

    // Save prompt for inspection
    const promptFile = join(DIFFS_DIR, `project-${project}-cycle-${state.cycleCount}-prompt.txt`);
    try { writeFileSync(promptFile, prompt, 'utf-8'); } catch {}

    // 3. Send to project's persistent process
    const proc = projectProcs.get(project);
    if (!proc) {
      log.error({ project }, 'Project cycle: no persistent process found');
      saveProjectState(project, state);
      return;
    }
    ...
    // Save reply for inspection
    const replyFile = join(DIFFS_DIR, `project-${project}-cycle-${state.cycleCount}-reply.txt`);
    try { writeFileSync(replyFile, reply, 'utf-8'); } catch {}

    log.info({ project, replyLen: reply.length, costUsd: costUsd.toFixed(4), model }, 'Project cycle Phase 2 complete');
    state.lastCycleTokens = { input: result.inputTokens || 0, output: result.outputTokens || 0, model, costUsd };

    // Persist project cycle cost to JSONL + SQLite (was missing — only main cycles recorded costs)
    if (costUsd > 0 && !config.costTrackingDisabled) {
      const costEntry = {
        type: 'project', model,
        inputTokens: result.inputTokens || 0, outputTokens: result.outputTokens || 0,
        cacheRead: result.cacheRead || 0, costUsd, durationMs: result.durationMs || 0,
        cronId: null, sessionId: proc?.sessionId || null,
      };
      try { appendFileSync(config.costsFile, JSON.stringify({ ...costEntry, project, ts: Date.now() }) + '\n'); } catch (e) { log.warn({ err: e.message, project }, 'Project cost JSONL write failed'); }
      try { insertCost({ ...costEntry, ts: Date.now() }); } catch (e) { log.warn({ err: e.message, project }, 'Project cost DB write failed'); }
      try { recordCostEntry(costUsd); } catch (e) { log.warn({ err: e.message, project }, 'Project cost analytics record failed'); }
      log.debug({ type: 'project', project, costUsd, model }, 'Project cost entry recorded');
    }

    // Track session size — reset when exceeding limits (prevents "Prompt is too long" accumulation)
    const projCycles = (projectSessionCycles.get(project) || 0) + 1;
    const projTokens = (projectSessionTokens.get(project) || 0) + (result.inputTokens || 0) + (result.outputTokens || 0);
    projectSessionCycles.set(project, projCycles);
    projectSessionTokens.set(project, projTokens);
    if (projTokens > SESSION_TOKEN_LIMIT || projCycles >= SESSION_CYCLE_LIMIT) {
      resetProjectSession(project, projTokens > SESSION_TOKEN_LIMIT ? 'token_limit' : 'cycle_limit');
    }
    recordProjectEvent(project, 'project:cycle:phase2', { model, costUsd, promptLen: prompt.length, replyLen: reply.length, promptFile });

    // 4. Parse + process response
    const parsed = parseAgentResponse(reply);

    // Route WA messages
    const quiet = isQuietHours();
    const moduleCats = getModuleMessageCategories();
    const msgCategory = (pc.signalPrefix && moduleCats[pc.signalPrefix]) || 'daily';
    for (const msg of parsed.waMessages) {
      if (quiet) {
        log.info({ project, msgLen: msg.length }, 'Project cycle: WA message suppressed (quiet hours)');
      } else {
        const sent = await sendToGroup(msgCategory, msg);
        if (sent) recordBotReply('project_cycle');
      }
    }
    ...
    // Log actions + Telegram notify
    if (parsed.actionsTaken.length > 0) {
      log.info({ project, actions: parsed.actionsTaken }, 'Project cycle: actions taken');
      recordProjectEvent(project, 'project:cycle:actions', { actions: parsed.actionsTaken, replyFile });
      wsEmit('project:cycle:actions', { project, actions: parsed.actionsTaken, replyFile });
      const summary = parsed.actionsTaken.map((a, i) => `${i + 1}. ${a.slice(0, 200)}`).join('\n');
      try { notify(`*[${project} cycle ${state.cycleCount}]*\n${summary.slice(0, 4000)}`); } catch {}
    }

    // Cross-cycle memory
    for (const sig of signals) recordRecentAction(sig.type, sig.summary || sig.type);
    for (const a of (parsed.actionsTaken || [])) recordRecentAction('action', a);

    // Module state writeback
    if (pc.stateKeyMap && Object.keys(pc.stateKeyMap).length > 0) {
      const stateKey = `module:${project}`;
      const handled = signals.filter(s => pc.stateKeyMap[s.type]).map(s => s.type);
      if (handled.length > 0) {
        const update = { lastCycleAt: Date.now() };
        for (const t of handled) update[pc.stateKeyMap[t]] = Date.now();
        setState(stateKey, update);
      }
    }
    ...
    if (parsed.goalCreates.length > 0) {
      try {
        ...
        const allGoals = listGoals({});
        const titleLower = gc.title.toLowerCase();
        const duplicate = allGoals.find(g => g.title.toLowerCase() === titleLower || g.title.toLowerCase().includes(titleLower) || titleLower.includes(g.title.toLowerCase()));
        if (!duplicate) {
          const agentGoals = allGoals.filter(g => g.source === 'agent' && (g.status === 'active' || g.status === 'in_progress'));
          if (agentGoals.length < 5) {
            const newGoal = addGoal(gc.title, { description: gc.description, source: 'agent', project });
            log.info({ project, goalId: newGoal.id, title: gc.title }, 'Project cycle: goal created');
          }
        }
      } catch (err) {
        log.warn({ project, err: err.message }, 'Project cycle: goal creation failed');
      }
    }
    ...
    for (const mc of (parsed.milestoneCompletes || [])) {
      try {
        const ms = completeMilestone(mc.goalId, mc.milestoneId, mc.evidence || null, model);
        if (ms) log.info({ project, goalId: mc.goalId, milestone: mc.milestoneId }, 'Project cycle: milestone completed');
      } catch {}
    }

    // Tool calls (project cycles always use persistent Claude)
    for (const tc of (parsed.toolCalls || [])) {
      try {
        const gate = gateAction('execute_tool', { targetExists: true, intentClarity: 0.7 });
        if (gate.action === 'ask') continue;
        const result = await executeTool(tc.name, tc.params);
        log.info({ project, tool: tc.name, success: result.success }, 'Project cycle: tool executed');
      } catch (err) {
        log.warn({ project, err: err.message, tool: tc.name }, 'Project cycle: tool execution error');
      }
    }
    ...
    for (const lesson of (parsed.lessonsLearned || [])) {
      try {
        const { recordLesson } = await import('./learning-journal.js');
        recordLesson(lesson, { action: 'project_cycle', outcome: 'observed', cycle: state.cycleCount, project });
      } catch {}
    }

    // Track goal phase transitions + cycle memory (project-cycle-injector)
    try {
      const projectGoals = listGoals({ status: ['active', 'in_progress'] }).filter(g => g.project === project);
      if (projectGoals.length > 0) detectPhaseTransitions(project, parsed, projectGoals);
      if (parsed.actionsTaken?.length > 0) {
        recordProjectCycleSummary(project, parsed.actionsTaken.map(a => a.slice(0, 200)).join('; ').slice(0, 1000), parsed.actionsTaken);
      }
    } catch (err) {
      log.warn({ project, err: err.message }, 'Phase transition detection failed');
    }

    // Save cycle diff
    try {
      saveCycleDiff(state.cycleCount, model, costUsd, parsed.actionsTaken, []);
    } catch {}

    state.consecutiveSpawns++;
    // Adaptive timing — productive project cycles re-cycle quickly
    let nextDelayMs = pc.cycleInterval;
    if ((parsed.actionsTaken.length >= 2 || parsed.goalCreates.length > 0) && state.consecutiveRecycles < MAX_CONSECUTIVE_RECYCLES) {
      nextDelayMs = RECYCLE_DELAY_MS;
      state.consecutiveRecycles++;
      log.info({ project, recycle: state.consecutiveRecycles }, 'Project cycle: productive — re-cycling in 2min');
    } else {
      state.consecutiveRecycles = 0;
    }

    saveProjectState(project, state);
    recordProjectEvent(project, 'project:cycle:complete', { cycleCount: state.cycleCount, actionCount: parsed.actionsTaken.length, followupCount: parsed.followups.length, replyFile });
    wsEmit('project:cycle:complete', { project, cycleCount: state.cycleCount, actionCount: parsed.actionsTaken.length, followupCount: parsed.followups.length, replyFile });

    // Quiet hours throttling
    if (isQuietHours() && !pc.hasUrgentWork()) {
      nextDelayMs = 60 * 60_000;
    }

    scheduleProjectNext(project, pc, nextDelayMs);
  } catch (err) {
    log.warn({ project, err: err.message }, 'Project cycle failed');
    recordProjectEvent(project, 'project:cycle:error', { error: err.message });
    try { logError('error', `agent-loop:${project}`, `Project cycle failed: ${err.message}`, err.stack); } catch {}
    // If prompt is too long, the session has grown too large — reset it immediately
    if (err.message?.toLowerCase().includes('prompt is too long') || err.message?.toLowerCase().includes('prompt too long')) {
      resetProjectSession(project, 'prompt_too_long');
    }
    saveProjectState(project, state);
    scheduleProjectNext(project, pc, pc.cycleInterval);
  } finally {
    projectRunning.set(project, false);
  }
}

function scheduleProjectNext(project, pc, delayMs) {
  if (stopped) return;
  // Clear existing timer for this project
  const existing = projectTimers.get(project);
  if (existing) clearTimeout(existing);

  projectNextCycleAt.set(project, Date.now() + delayMs);
  const timer = setTimeout(() => {
    projectTimers.delete(project);
    runProjectCycle(pc).catch(err => {
      log.error({ project, err: err.message }, 'Project cycle unhandled error');
      try { logError('critical', `agent-loop:${project}`, `Unhandled: ${err.message}`, err.stack); } catch {}
    });
  }, delayMs);
  timer.unref();
...
export function startAgentLoop(send, queue, { skipCore = false } = {}) {
  sendFn = send;
  queueRef = queue || null;
  stopped = false;

  // ── Startup cost cap check — prevent scheduling if already over budget ──
  const STARTUP_CAP = parseFloat(process.env.DAILY_COST_LIMIT || '50');
  const startupCost = getTodayCost();
  if (startupCost >= STARTUP_CAP) {
    log.warn({ todayCost: startupCost.toFixed(2), cap: STARTUP_CAP }, 'Agent loop startup BLOCKED — daily cost cap already exceeded. No cycles will be scheduled.');
    stopped = true;
    // Schedule a re-check at midnight (Israel time) to resume next day
    const now = new Date();
    const midnight = new Date(now.toLocaleDateString('en-CA', { timeZone: config.timezone }) + 'T00:00:00+02:00');
    midnight.setDate(midnight.getDate() + 1);
    const msUntilMidnight = midnight.getTime() - Date.now();
    const resumeTimer = setTimeout(() => {
      log.info('Daily cost cap reset — restarting agent loop');
      stopped = false;
      startAgentLoop(send, queue, { skipCore });
    }, msUntilMidnight);
    resumeTimer.unref();
    return;
  }
  log.info({ todayCost: startupCost.toFixed(2), cap: STARTUP_CAP, remaining: (STARTUP_CAP - startupCost).toFixed(2) }, 'Startup cost check passed');

  // Initialize pluggable LLM backends (Ollama, NIM, env-discovered)
  initBackends();

  // Register progressive disclosure tools (lazy context loading)
  registerContextTools();

  if (!skipCore) {
    // Spawn core persistent Claude process (uses stripped MCP config — no pixellab/elevenlabs)
    coreProc = new PersistentProcess('agent-core', randomUUID(), AGENT_SESSION_SYSTEM_PROMPT, {
      model: SONNET_MODEL,
      timeout: config.cliTimeoutHattrick,         // 30 min (Hattrick MCP can be slow)
      activityTimeout: config.cliActivityTimeout,
      cacheKeepAlive: true,
      mcpConfig: config.mcpConfigCorePath,
    });
  }

  // Spawn per-project persistent processes (from module manifests)
  for (const pc of getAllProjectConfigs()) {
    const proc = new PersistentProcess(`agent-${pc.name}`, randomUUID(), AGENT_SESSION_SYSTEM_PROMPT, {
      model: pc.model || SONNET_MODEL,
      timeout: config.cliTimeout,
      activityTimeout: config.cliActivityTimeout,
      cacheKeepAlive: true,
      mcpConfig: pc.mcpConfig,
    });
    projectProcs.set(pc.project || pc.name, proc);
    log.info({ project: pc.project || pc.name, mcpConfig: pc.mcpConfig }, 'Project persistent process spawned');
  }

  if (!skipCore) {
    // First core cycle after 2 minutes (let the bot stabilize)
    ...
    log.info('Core agent cycle SKIPPED (skipCore=true)');
  }

  // Schedule independent project cycles (staggered start to avoid thundering herd)
  let staggerIdx = 0;
  for (const pc of getAllProjectConfigs()) {
    if (pc.cycleInterval) {
      const project = pc.project || pc.name;
      const stagger = (3 + staggerIdx * 2) * 60_000; // 3min, 5min, 7min, ...
      scheduleProjectNext(project, pc, stagger);
      log.info({ project, intervalMin: pc.cycleInterval / 60_000, firstRunIn: `${stagger / 60_000}min` }, 'Project cycle scheduled');
      staggerIdx++;
    }
  }

  log.info({ intervalMin: INTERVAL_MS / 60_000, routineModel: ROUTINE_MODEL, sonnetModel: SONNET_MODEL, alwaysThinkEvery: ALWAYS_THINK_EVERY, projectCycles: staggerIdx, skipCore }, 'Agent loop started (autonomous)');
}
...
export function pauseMainCycle() {
  if (mainPaused) return { paused: true, already: true };
  mainPaused = true;
  log.info('Main cycle paused');
  return { paused: true };
}

export function resumeMainCycle() {
  if (!mainPaused) return { paused: false, already: true };
  mainPaused = false;
  log.info('Main cycle resumed');
  return { paused: false };
}

export function pauseProjectCycle(projectId) {
  const pcConfigs = getAllProjectConfigs();
  const pc = pcConfigs.find(p => (p.project || p.name) === projectId);
  if (!pc) return { error: 'project_not_found' };
  const project = pc.project || pc.name;
  if (projectPaused.get(project)) return { paused: true, already: true };
  projectPaused.set(project, true);
  log.info({ project }, 'Project cycle paused');
  return { paused: true };
}

export function resumeProjectCycle(projectId) {
  const pcConfigs = getAllProjectConfigs();
  const pc = pcConfigs.find(p => (p.project || p.name) === projectId);
  if (!pc) return { error: 'project_not_found' };
  const project = pc.project || pc.name;
  if (!projectPaused.get(project)) return { paused: false, already: true };
  projectPaused.set(project, false);
  log.info({ project }, 'Project cycle resumed');
  return { paused: false };
}
...
export function stopAgentLoop() {
  ...
  // Clear all project cycle timers
  for (const [project, timer] of projectTimers) {
    clearTimeout(timer);
    log.info({ project }, 'Project cycle timer cleared');
  }
  projectTimers.clear();
  projectNextCycleAt.clear();
  projectRunning.clear();
  projectLiveEvents.clear();

  coreProc?.shutdown();
  coreProc = null;
  for (const [name, proc] of projectProcs) {
    proc.shutdown();
    log.info({ project: name }, 'Project process shut down');
  }
  projectProcs.clear();
...
export function getAgentLoopStatus() {
  ...
  return {
    ...
    mode: 'autonomous',
    agentSession: {
      alive: coreProc?.getStats()?.alive || false,
      cycles: agentSessionCycles,
      tokens: agentSessionTokens,
      limit: SESSION_TOKEN_LIMIT,
      messageCount: coreProc?.getStats()?.messageCount || 0,
    },
    projectProcesses: Object.fromEntries(
      [...projectProcs.entries()].map(([name, proc]) => [name, proc.getStats()])
    ),
    projectCycles: Object.fromEntries(
      getAllProjectConfigs().filter(pc => pc.cycleInterval).map(pc => {
        const project = pc.project || pc.name;
        const pState = loadProjectState(project);
        return [project, {
          intervalMin: pc.cycleInterval / 60_000,
          running: projectRunning.get(project) || false,
          paused: projectPaused.get(project) || false,
          cycleCount: pState.cycleCount || 0,
          lastCycleAt: pState.lastCycleAt ? new Date(pState.lastCycleAt).toISOString() : null,
          nextCycleAt: projectNextCycleAt.get(project) ? new Date(projectNextCycleAt.get(project)).toISOString() : null,
          lastSignals: pState.lastSignals || [],
          pendingFollowups: pState.pendingFollowups?.length || 0,
          lastCycleTokens: pState.lastCycleTokens || null,
        }];
      })
    ),
  };
...
export function getAgentLoopDetail() {
  ...
  return {
    ...
    qmdSync: getQmdSyncStatus(),
  };
}

export function triggerProjectCycleNow(projectId) {
  if (stopped) return { triggered: false, reason: 'loop_stopped' };
  const pcConfigs = getAllProjectConfigs();
  const pc = pcConfigs.find(p => (p.project || p.name) === projectId);
  if (!pc) return { triggered: false, reason: 'project_not_found' };
  const project = pc.project || pc.name;
  if (projectRunning.get(project)) return { triggered: false, reason: 'cycle_already_running' };
  // Clear existing timer
  const existing = projectTimers.get(project);
  if (existing) clearTimeout(existing);
  projectTimers.delete(project);
  projectNextCycleAt.set(project, Date.now());
  runProjectCycle(pc).catch(err => {
    log.error({ project, err: err.message }, 'Project cycle manual trigger error');
  });
  log.info({ project }, 'Project cycle manually triggered');
  return { triggered: true };
}

export function getProjectCycleDetail(projectId) {
  const pcConfigs = getAllProjectConfigs();
  const pc = pcConfigs.find(p => (p.project || p.name) === projectId);
  if (!pc) return null;
  const project = pc.project || pc.name;
  ...
  return {
    project,
    intervalMin: pc.cycleInterval ? pc.cycleInterval / 60_000 : null,
    running: projectRunning.get(project) || false,
    paused: projectPaused.get(project) || false,
    cycleCount: pState.cycleCount || 0,
    lastCycleAt: pState.lastCycleAt ? new Date(pState.lastCycleAt).toISOString() : null,
    nextCycleAt: projectNextCycleAt.get(project) ? new Date(projectNextCycleAt.get(project)).toISOString() : null,
    consecutiveSpawns: pState.consecutiveSpawns || 0,
    lastSignals: pState.lastSignals || [],
    pendingFollowups: pState.pendingFollowups || [],
    lastCycleTokens: pState.lastCycleTokens || null,
    process: proc ? proc.getStats() : null,
    recentEvents: (projectLiveEvents.get(project) || []).slice(-MAX_PROJECT_EVENTS),
  };
}
