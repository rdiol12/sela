...
import { listGoals, getStaleGoals, getUpcomingDeadlines, addGoal, updateGoal, completeMilestone, getGoalsContext, getJsonMtime, importJsonChanges, load as reloadGoals } from './goals.js';
import { listCrons } from './crons.js';
import { emit as wsEmit } from './ws-events.js';
import config from './config.js';
import { getCostOverview, rollupOldCosts, recordCostEntry } from './cost-analytics.js';
...
function qmdSyncIfDue(state) {
  ...
  const proc = exec(`"${qmdBin}" update`, { timeout: 15 * 60_000 }, (err) => {
    if (err) {
    ...
    const proc2 = exec(`"${qmdBin}" embed`, { timeout: 20 * 60_000 }, (err2) => {
      qmdSync.running = false;
      qmdSync.lastDurationMs = Date.now() - qmdSync.startedAt;
      qmdSync.pid = null;
      if (err2) {
...
function collectSignals(state) {
  ...
  // 3D asset generation pipeline (Shattered Crown — Blender MCP)
  signals.push(...detectAssetGeneration());
  ...
  try {
    ...
    if (costUsd > 0 && !config.costTrackingDisabled) {
      const costEntry = {
        ...
        cronId: null, sessionId: agentProc?.sessionId || null,
      };
      try { appendFileSync(config.costsFile, JSON.stringify({ ...costEntry, ts: Date.now() }) + '\n'); } catch {}
      try { insertCost({ ...costEntry, ts: Date.now() }); } catch {}
      try { recordCostEntry(costUsd); } catch {}
    }
