...
/**
 * Detect pending 3D assets in the Shattered Crown asset manifest.
 *
 * Fires when:
 *   - asset-manifest.json exists and has pending assets
 *   - Blender MCP server is connected (checked via mcp-gateway health)
 *   - Cooldown: at most one signal per 30 minutes (asset generation takes time)
 *
 * The agent-loop handler calls lib/asset-pipeline.js generateOneAsset().
 */
let _lastAssetSignalAt = 0;
const ASSET_SIGNAL_COOLDOWN_MS = 30 * 60 * 1000; // 30 minutes

export function detectAssetGeneration() {
  const signals = [];
  try {
    const nowMs = Date.now();
    ...
    if (pending === 0) return signals; // all done

    const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
    const urgency = pending > 40 ? 'medium' : pending > 20 ? 'low' : 'low';
    ...
    log.warn({ err: err.message }, 'detectAssetGeneration: failed');
  }
  return signals;
}
