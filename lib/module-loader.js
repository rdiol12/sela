...
let projectProcessConfigs = [];           // [{ name, project, mcpConfig, systemPrompt, model }]

// ─── Startup ─────────────────────────────────────────────────────────────────

export async function loadModules({ skip = [] } = {}) {
  ...
  if (skip.length) {
    ...
    log.info({ skip, remaining: dirs }, 'Module skip list applied');
  }
  ...
  for (const dirName of dirs) {
    ...
    try {
      ...
      // Register project-specific persistent process config (carries full cycle context)
      if (manifest.projectProcess) {
        projectProcessConfigs.push({
          name: manifest.name,
          ...manifest.projectProcess,
          detectSignals: manifest.detectSignals || null,
          briefBuilders: manifest.briefBuilders || {},
          sonnetSignalTypes: new Set(manifest.sonnetSignalTypes || []),
          stateKeyMap: manifest.stateKeyMap || {},
          signalPrefix: manifest.signalPrefix || '',
          hasUrgentWork: manifest.hasUrgentWork || (() => false),
        });
      }
...
export function getProjectProcessConfigs() { return projectProcessConfigs; }
