  ...
  try {
    ...
    if (sendFn && proposals.length > 0) {
      ...
      try {
        ...
        // Mark newly-sent proposals so the re-send block below doesn't duplicate them
        const now = Date.now();
        const allProposals = loadProposals();
        for (const p of allProposals) {
          if (p.status === 'pending' && !p.sentAt) p.sentAt = now;
        }
        saveProposals(allProposals);
      } catch (err) {
        log.warn({ err: err.message }, 'Failed to send proposals via WA');
      }
    }

    // Re-send any pending proposals that weren't delivered (e.g. bot was restarting)
    // Cooldown: only re-send a proposal if it hasn't been attempted in the last 2 hours
    const RESEND_COOLDOWN_MS = 2 * 3600_000;
    if (sendFn) {
      const allProposals = loadProposals();
      const now = Date.now();
      const undelivered = allProposals.filter(
        p => p.status === 'pending' && !p.sentAt && (now - (p.lastResendAt || p.createdAt)) > RESEND_COOLDOWN_MS
      );
      if (undelivered.length > 0) {
        try {
          const batched = undelivered.map(p => p.message).join('\n\n---\n\n');
          await sendFn(batched);
          for (const p of undelivered) { p.sentAt = now; p.lastResendAt = now; }
          log.info({ count: undelivered.length }, 'Re-sent undelivered proposals (batched)');
        } catch (err) {
          // Track attempt time even on failure to enforce cooldown
          for (const p of undelivered) p.lastResendAt = now;
          log.warn({ err: err.message }, 'Failed to re-send undelivered proposals');
        }
      }
      saveProposals(allProposals);
    }
