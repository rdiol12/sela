...
async function checkLoop() {
  ...
  // UE5 autonomous game builder (non-blocking, runs when UE5 is open)
  try {
    const { runUnrealAutonomy } = await import('./unreal-autonomy.js');
    runUnrealAutonomy().catch(err => log.warn({ err: err.message }, 'unreal autonomy error'));
  } catch (err) {
    log.warn({ err: err.message }, 'unreal autonomy import failed');
  }
