...
const INTENTS = [
  ...
  // ── buildgame ────────────────────────────────────────────────────────────
  {
    name: 'buildgame',
    patterns: [
      { re: /^build\s*(the\s*)?(game|shattered|ue5|unreal)[\s!.]*$/i, tier: 'strong' },
      { re: /\bbuild\s*(everything|all|playable|full)\s*(game|level|ue5|unreal)?\b/i, tier: 'strong' },
      { re: /\b(make|create|build)\s*(it|the game)?\s*playable\b/i, tier: 'strong' },
      { re: /\b(i want to|let me|can you)\s*(play|test)\s*(the\s*)?(game|shattered crown)\b/i, tier: 'strong' },
      { re: /\bbuild\s*(bp_kael|kael|character)\b/i, tier: 'strong' },
      { re: /\bstart\s*building\s*(the\s*)?(game|level|world)\b/i, tier: 'medium' },
      // Hebrew
      { re: /בנה?\s*(את\s*)?המשחק/, tier: 'strong' },
      { re: /(?:אני\s*)?רוצה\s*לשחק/, tier: 'strong' },
    ],
    extract: () => ({}),
  },
