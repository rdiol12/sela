...
/**
 * Get today's running cost total (for enforcement).
 * Always reads from SQLite for accuracy — the in-memory cache can drift
 * after process restarts or when costs are recorded outside recordCostEntry().
 */
export function getTodayCost() {
  ...
  try {
    ...
    todayCache = { date: today, total, count: row.count || 0, loaded: true };
    return total;
  } catch (err) {
    log.warn({ err: err.message }, 'getTodayCost SQLite read failed — using cache');
    ensureTodayCache();
    return todayCache.total;
  }
}
