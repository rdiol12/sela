...
import { buildContextBlock } from './code-index.js';
...
export function buildMilestoneBrief(goal, milestone) {
  // Inject relevant file context so the agent doesn't waste tokens exploring
  const contextBlock = buildContextBlock(milestone.title, goal.description);
  ...
  return [
    ...
    `1. Check the relevant files listed above before writing new code — extend, don't duplicate.`,
    ...
    `   <tool_call name="shell_exec">{"command": "node scripts/syntax-check.cjs", "timeout": 60000}</tool_call>`,
...
export async function runTests() {
  // Step 1: Syntax check via .cjs script — avoids ESM/CJS and shell-escaping issues
  const syntax = await executeTool('shell_exec', {
    command: `cd "${PROJECT_ROOT}" && node scripts/syntax-check.cjs`,
    timeout: 60000,
  });
