...
class ServerConnection {
  constructor(name, serverConfig) {
    this.name = name;
    this.serverConfig = serverConfig; // { command, args, env } or { url, transport, headers }
    this.isHttp = !!(serverConfig.url && (serverConfig.transport === 'http' || serverConfig.type === 'streamable-http'));
    this.client = null;
    this.transport = null;
  ...
  async connect() {
    ...
    try {
      ...
      if (this.isHttp) {
        // Remote HTTP MCP server (e.g. PixelLab)
        const requestInit = {};
      ...
      this.client.onerror = (err) => {
        // Throttle repeated client errors to once per 5 min to avoid log flooding
        const now = Date.now();
        if (now - this._lastClientErrorLog > 300_000) {
          this._lastClientErrorLog = now;
      ...
      // Route stderr through logger (stdio only)
      if (!this.isHttp) {
        const stderrStream = this.transport.stderr;
        if (stderrStream) {
          let buf = '';
          // Rate-limit repeated stderr errors to avoid log flooding (e.g. blender-mcp connection refused)
          const stderrErrorCounts = {};
          const STDERR_ERROR_THROTTLE_MS = 300_000; // log same error at most once per 5 min
          stderrStream.on('data', (chunk) => {
            ...
            for (const line of lines) {
              ...
              if (trimmed.includes('ERROR')) {
                // Throttle repeated stderr errors — use first 80 chars as dedup key
                const key = trimmed.slice(0, 80);
                const now = Date.now();
                const last = stderrErrorCounts[key] || 0;
                if (now - last > STDERR_ERROR_THROTTLE_MS) {
                  stderrErrorCounts[key] = now;
                  log.warn({ server: this.name, stderr: trimmed }, 'MCP stderr');
                }
              } else {
                log.debug({ server: this.name, stderr: trimmed }, 'MCP stderr');
              }
            }
          });
        }
      ...
      log.info({ server: this.name }, `Connected to MCP server: ${this.name} (${this.isHttp ? 'http' : 'stdio'})`);
    } finally {
      this.reconnecting = false;
    }
  }
...
function expandEnvVars(obj) {
  ...
  for (const [k, v] of Object.entries(obj)) {
    if (typeof v === 'string' && v.includes('${')) {
      obj[k] = v.replace(/\$\{([^}]+)\}/g, (_, envKey) => process.env[envKey] || '');
...
function loadServerConfigs() {
  ...
  try {
    ...
    for (const cfg of Object.values(servers)) {
      if (cfg.env) expandEnvVars(cfg.env);
    }
  } catch (err) {
    log.warn({ err: err.message }, 'Failed to read mcp-config.json');
  }
  // Merge supplementary configs (HTTP transports not supported by Claude CLI)
  for (const suffix of ['-pixellab.json', '-ludo.json']) {
    try {
      const extraPath = config.mcpConfigPath.replace('.json', suffix);
      const extra = JSON.parse(readFileSync(extraPath, 'utf-8'));
      const extraServers = extra.mcpServers || {};
      for (const cfg of Object.values(extraServers)) {
        if (cfg.env) expandEnvVars(cfg.env);
        if (cfg.headers) expandEnvVars(cfg.headers);
      }
      Object.assign(servers, extraServers);
    } catch { /* optional file */ }
  }
  return servers;
}
...
export async function init() {
  const configs = getServerConfigs();
  const names = Object.keys(configs).filter(n => !SKIP_SERVERS.has(n));

  log.info({ servers: names }, `Connecting to ${names.length} MCP servers`);

  const results = await Promise.allSettled(
    names.map(async (name) => {
      try {
        const conn = getServer(name);
        await conn.connect();
        if (name === 'vestige') conn.startHealthCheck();
        return { name, ok: true };
      } catch (err) {
        log.warn({ server: name, err: err.message }, `Failed to connect to ${name} MCP at startup`);
        return { name, ok: false, err: err.message };
      }
    }),
  );
  ...
  log.info({ connected, failed, total: names.length }, `MCP init complete — ${connected}/${names.length} servers connected`);
}
