...
export function listProjects() {
  ...
  return projectGoals.map(pg => {
    ...
    const slug = meta.slug || pg.id;
    const agentCycle = getProjectAgentCycle(slug) || { enabled: false };
    return {
      ...pg,
...
export function addGoalToProject(projectId, goalTitle, { description, priority, milestones } = {}) {
  ...
  // Auto-generate milestones if none provided — the agent-loop requires at least
  // one pending milestone to generate a goal_work signal.
  const ms = (milestones && milestones.length > 0) ? milestones : autoMilestones(goalTitle);
  ...
  wsEmit('goals:updated', { count: 1 });
  log.info({ projectId: project.id, goalId: child.id, title: goalTitle, milestonesGenerated: ms.length }, 'Goal added to project');
  return child;
}

/**
 * Generate default milestones from a goal title.
 * Gives the agent-loop something to pick up immediately.
 */
function autoMilestones(title) {
  const t = title.toLowerCase();
  const ms = [];

  // Always start with research/planning
  ms.push(`Research and plan: ${title}`);

  // Detect common task patterns and add specific milestones
  if (t.includes('add') || t.includes('create') || t.includes('implement') || t.includes('build')) {
    ms.push(`Implement: ${title}`);
  }
  if (t.includes('fix') || t.includes('bug') || t.includes('error')) {
    ms.push(`Diagnose root cause`);
    ms.push(`Apply fix and verify`);
  }
  if (t.includes('test') || t.includes('verify')) {
    ms.push(`Write tests and verify`);
  }

  // Always end with completion verification
  ms.push('Verify completion and update progress');
...
function countWorkspaceFiles(pathOrSlug) {
  ...
  try {
    const dir = isAbsolute(pathOrSlug)
      ? pathOrSlug
...
// ── Agent Cycle Config ──────────────────────────────────────────────────────
// Per-project agent cycle settings stored in kv_state as `project-cycle:{slug}`.

export function getProjectAgentCycle(slug) {
  const raw = kvGet(`project-cycle:${slug}`);
  return raw ? JSON.parse(raw) : null;
}

export function setProjectAgentCycle(slug, { enabled, intervalMin = 10, model = 'sonnet' }) {
  const existing = getProjectAgentCycle(slug) || {};
  const config = {
    ...existing,
    enabled: !!enabled,
    intervalMin: Math.max(1, Math.min(60, intervalMin)),
    model,
    updatedAt: Date.now(),
  };
  kvSet(`project-cycle:${slug}`, JSON.stringify(config));
  log.info({ slug, ...config }, 'Project agent cycle config updated');
  return config;
}

/**
 * List all projects that have agent cycling enabled.
 * Returns configs suitable for the agent-loop's project cycle system.
 */
export function listAgentCycleProjects() {
  ...
  for (const p of projects) {
    ...
    if (cycle && cycle.enabled) {
      results.push({
        project: slug,
        title: p.title,
        cycleInterval: (cycle.intervalMin || 10) * 60_000,
        model: cycle.model || 'sonnet',
        mcpConfig: join(process.cwd(), 'mcp-config-core.json'),
        // No detectSignals — dynamic projects use goal_work + followups only
      });
    }
  }
  return results;
