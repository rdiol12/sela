...
function goalsToDb(goal) {
  const db = getDb();
  db.prepare(`
    INSERT INTO goals (id, title, description, status, priority, progress, milestones, log, linked_topics, category, parent_goal_id, project, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
export function load() {
  try {
    ...
    // Auto-classify untagged goals into projects
    classifyExistingGoals();
  } catch (err) {
    goals = [];
...
const PROJECT_CLASSIFIERS = {
  maplestory:       /maple|cosmic|npc.*script|wz\b|mob.*stat|quest.*giver|server.*v83|drop.*table/i,
  'shattered-crown': /shattered.*crown|unreal|ue5|region.*build|blueprint|volumetric|wayshrine|corruption|codex|rival.*crown|shard.*echo|photo.*mode|motion.*match/i,
  hattrick:         /hattrick|match.*prep|transfer.*market|player.*training|squad|lineup|economy.*check/i,
  sela:             /\bsela\b|bot.*infra|whatsapp|agent.*loop|persistent.*mode|memory.*tier|prompt.*assembl|context.*management|auto.?coder/i,
};

// Shattered Crown parent goal ID (for child goals)
const SHATTERED_CROWN_PARENT = 'ae6377c6';

function classifyExistingGoals() {
  let classified = 0;
  for (const g of goals) {
    if (g.project) continue; // already classified

    // Check parent goal
    if (g.parentGoalId === SHATTERED_CROWN_PARENT) {
      g.project = 'shattered-crown';
    ...
    // Regex match on title + description + linkedTopics
    const text = `${g.title} ${g.description || ''} ${(g.linkedTopics || []).join(' ')} ${g.category || ''}`;
    for (const [project, re] of Object.entries(PROJECT_CLASSIFIERS)) {
      if (re.test(text)) {
        g.project = project;
        g.updatedAt = Date.now();
        goalsToDb(g);
  ...
  if (classified > 0) {
    syncJsonFromDb();
    log.info({ classified }, 'Auto-classified goals into projects');
  }
}
...
export function listGoals(filter = {}) {
  ...
  if (filter.project) {
    result = result.filter(g => g.project === filter.project);
  }
...
export function addGoal(title, { description = '', priority = 'normal', category = 'project', deadline = null, linkedTopics = [], milestones = [], source = 'user', parentGoalId = null, project = null } = {}) {
...
export function updateGoal(idOrTitle, fields) {
  ...
  const updatable = ['title', 'description', 'priority', 'category', 'deadline', 'linkedTopics', 'parentGoalId', 'project'];
 ...
 */
export function getGoalsContext(projectFilter = null, excludeProjects = null) {
  let active = listGoals({ status: ['active', 'in_progress', 'blocked'] });
  if (projectFilter) active = active.filter(g => g.project === projectFilter);
  if (excludeProjects?.size) active = active.filter(g => !g.project || !excludeProjects.has(g.project));
  ...
  for (const g of active) {
    const proj = g.project || 'general';
    if (!byProject[proj]) byProject[proj] = [];
    byProject[proj].push(g);
  }

  const lines = [];
  for (const [proj, projGoals] of Object.entries(byProject)) {
    lines.push(`[${proj}]`);
    for (const g of projGoals) {
      ...
      lines.push(`  - ${prio} ${g.title} (${g.progress}%${dl}${nextStr}) [${g.status}]`);
    }
  }
  return lines.join('\n');
}
...
/**
 * Compact goals context — titles + progress only, no milestones or descriptions.
 * Used by progressive disclosure in agent-loop to reduce prompt size.
 * Agent can request full details via load_goal_context tool when needed.
 * Saves ~200-500 tokens per cycle vs full getGoalsContext().
 */
export function getGoalsContextCompact(projectFilter = null, excludeProjects = null) {
  let active = listGoals({ status: ['active', 'in_progress', 'blocked'] });
  if (projectFilter) active = active.filter(g => g.project === projectFilter);
  if (excludeProjects?.size) active = active.filter(g => !g.project || !excludeProjects.has(g.project));
  if (active.length === 0) return '';
  ...
  for (const [proj, projGoals] of Object.entries(byProject)) {
    lines.push(`[${proj}]`);
    for (const g of projGoals) {
      const prio = g.priority ? `[${g.priority.toUpperCase()}]` : '';
      lines.push(`  - ${prio} ${g.title} (${g.progress}%) [${g.status}]`);
    }
  }
  lines.push(`\n_Use <tool_call name="file_read">{"path":"data/goals.json"}</tool_call> for full goal details._`);
  return lines.join('\n');
}
