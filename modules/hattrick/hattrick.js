...
import * as economyModeling from './economy-modeling.js';
...
### MCP Tool Error Handling & Cloudflare Bypass
- If an MCP tool returns an error, empty data, "Performing security verification", or a login/session page, **retry once**.
- If it fails again with Cloudflare challenge or browser error (exitCode=21), **use the Cloudflare bypass**:
  Call \`mcp__scrapling__stealthy_fetch\` with: url="https://www.hattrick.org/en/<path>", solve_cloudflare=true, timeout=45000
  This bypasses Cloudflare via Scrapling's stealth browser. Parse the returned HTML for the data you need.
- If BOTH MCP tools AND stealthy_fetch fail, report in <wa_message>: "All Hattrick access methods failed — both MCP and Cloudflare bypass."
- If hattrick_get_players returns empty/zero players, session expired. Try stealthy_fetch first, then report: "Hattrick session expired — the user, run hattrick_login."
...
- If it fails again (Cloudflare block, exitCode=21, timeout), **fallback to stealthy_fetch**:
  Call \`mcp__scrapling__stealthy_fetch\` with: url="https://www.hattrick.org/en/<path>", solve_cloudflare=true
  Parse the returned HTML directly for the data you need.
- If all methods fail, report the failure in <wa_message> — do NOT guess or hallucinate.
- Empty player list = session expired. Try stealthy_fetch bypass before reporting.
...
export function buildEconomyCrisisModelingBrief(signal) {
  return `## Hattrick Economy Crisis Scenario Modeling

${HT_BASE}

### Task:
Build 4-scenario financial projections + 2-week action plan.

1. Use get_economy to fetch CURRENT financial data (cash, weekly income/expenses)
2. Import buildScenarios, build2WeekActionPlan from economy-modeling.js
3. Run: buildScenarios({ cash, weeklyIncome, weeklyExpenses })
4. Run: build2WeekActionPlan(scenarios) for 2-week timeline
5. Format reports using formatScenarioReport() + formatActionPlan()
6. Output:
   - **Scenario Report**: 4-week cash projections for all 4 scenarios
   - **Action Plan**: Prioritized 2-week actions (cost-cutting + player sales)
   - **Analysis**: Which scenario is best, warnings, recommendations

Scenarios:
- **Status Quo**: No changes (establish baseline)
- **Salary Cuts**: 10% wage reduction (modest), 4-week runway
- **Player Sales**: Liquidate 5 young fringe players (~500K), strong cash boost
- **Hybrid**: 5% cuts + 2 player sales (balanced)

Format as markdown tables. Save as hattrick-economy-scenarios.json in data/state/.`;
}
...
6. Compute bid: bidAmount = currentHighestBid + 5000. EXACTLY. Nothing more. (Hattrick enforces 5,000 NIS minimum increment)
...
- Your bid = currentHighestBid + 5,000 NIS. EXACTLY. No exceptions. (Hattrick enforces 5,000 NIS MINIMUM increment — bids below this are rejected by the platform)
- NEVER bid your "estimate" of what a player is worth.
- NEVER bid a round number like 100K, 200K, 250K unless that equals currentHighestBid + 5000.
...
- If outbid later, the next cycle will re-bid at their bid + 5000. This is by design.
...
export function computeBidAmount(player, auctionState, estimatedValue) {
  ...
  const MIN_INCREMENT = 5000; // Hattrick enforces minimum 5,000 NIS bid increment (verified 08.03.2026)
  ...
  if (minutesRemaining <= 30) {
    ...
    // Round UP to nearest 5000 to satisfy platform minimum
    bidAmount = Math.ceil(bidAmount / 5000) * 5000;
  } else {
    bidAmount = currentHighestBid + MIN_INCREMENT;
  }
...
export default {
  ...
  // Economy modeling (crisis scenario analysis)
  ...economyModeling,
};
