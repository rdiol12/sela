...
async def _open_page(target_url: str, page_callback=None, wait_ms: int = 3000) -> dict:
    ...
    # Try StealthyFetcher first, fall back to raw Playwright
    stealth_ok = False
    try:
    ...
    # Fallback: raw Playwright when StealthyFetcher crashes
    if not stealth_ok:
        try:
            ...
            captured["error"] = f"Both StealthyFetcher and Playwright failed: {e2}"
